...
class DatabaseUtilitiesGUI(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.nb.add(self.fix_missing_locations_frame,
                    text='Fix Missing Locations')
        self.nb.add(self.validate_geojson_frame, text='Validate GeoJSON')
        self.nb.add(self.recalculate_distances_frame,
                    text='Recalculate Distances')
    ...
    def create_stats_tab(self):
        ttk.Label(self.stats_frame, text="Collection Document Counts:",
                  font=('Helvetica', 14)).pack(pady=10)
        self.stats_text = tk.Text(self.stats_frame, height=10, width=50)
        self.stats_text.pack()
        self.refresh_stats_button = ttk.Button(
            self.stats_frame, text="Refresh Stats", command=self.view_collection_stats)
    ...
    def create_view_tab(self):
        ttk.Label(self.view_frame, text="Select a collection to view samples:", font=(
        ...
        self.view_button = ttk.Button(
            self.view_frame, text="View Data", command=self.view_data)
    ...
    def view_data(self):
        ...
        try:
            ...
            if samples:
                for doc in samples:
                    self.view_text.insert(tk.END, json.dumps(
                        doc, default=str, indent=4) + "\n\n")
            else:
                self.view_text.insert(
                    tk.END, "No documents found in this collection.")
        except Exception as e:
            self.view_text.insert(tk.END, f"Error: {e}")

    def create_clear_tab(self):
        ttk.Label(self.clear_frame, text="Select a collection to clear:",
                  font=('Helvetica', 14)).pack(pady=10)
        ...
        self.preview_clear_button = ttk.Button(
            self.clear_frame, text="Preview Data", command=self.preview_clear_data)
        self.preview_clear_button.pack(pady=5)
        self.clear_button = ttk.Button(
            self.clear_frame, text="Clear Collection", command=self.clear_collection)
    ...
    def preview_clear_data(self):
        ...
        try:
            samples = list(collection.find().limit(5))
            if samples:
                self.clear_text.insert(
                    tk.END, "Sample documents to be deleted:\n\n")
                for doc in samples:
                    self.clear_text.insert(tk.END, json.dumps(
                        doc, default=str, indent=4) + "\n\n")
            else:
                self.clear_text.insert(
                    tk.END, "No documents found in this collection.")
    ...
    def clear_collection(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to clear the '{collection_name}' collection?")
        if confirm:
            threading.Thread(target=self._clear_collection_thread,
    ...
    def _clear_collection_thread(self, collection):
        try:
            ...
            messagebox.showinfo(
                "Success", f"Deleted {result.deleted_count} documents from '{collection.name}'.")
            self.view_collection_stats()
        except Exception as e:
            messagebox.showerror("Error", f"Error clearing collection: {e}")

    def create_delete_date_range_tab(self):
        ttk.Label(self.delete_date_range_frame, text="Delete Trips within Date Range", font=(
            'Helvetica', 14)).pack(pady=10)
        ttk.Label(self.delete_date_range_frame,
                  text="Select Collection:").pack()
        ...
        ttk.Label(self.delete_date_range_frame,
                  text="Start Date (YYYY-MM-DD):").pack(pady=5)
        self.delete_start_date_entry = ttk.Entry(self.delete_date_range_frame)
        self.delete_start_date_entry.pack()

        ttk.Label(self.delete_date_range_frame,
                  text="End Date (YYYY-MM-DD):").pack(pady=5)
        self.delete_end_date_entry = ttk.Entry(self.delete_date_range_frame)
        self.delete_end_date_entry.pack()

        self.preview_delete_date_button = ttk.Button(
            self.delete_date_range_frame, text="Preview Deletion", command=self.preview_delete_date_range)
        self.preview_delete_date_button.pack(pady=5)
        self.delete_date_button = ttk.Button(
            self.delete_date_range_frame, text="Delete", command=self.delete_date_range)
        self.delete_date_button.pack(pady=5)

        self.delete_date_text = ScrolledText(
            self.delete_date_range_frame, height=20, width=90)
    ...
    def preview_delete_date_range(self):
        ...
        if samples:
            self.delete_date_text.insert(
                tk.END, f"Total documents to delete: {total_count}\n\nSample documents:\n\n")
            for doc in samples:
                self.delete_date_text.insert(tk.END, json.dumps(
                    doc, default=str, indent=4) + "\n\n")
        else:
            self.delete_date_text.insert(
                tk.END, "No documents found in the specified date range.")
    ...
    def delete_date_range(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to delete documents from '{collection_name}' between {start_date_str} and {end_date_str}?")
        if confirm:
            threading.Thread(target=self._delete_date_range_thread, args=(
                collection, start_date, end_date)).start()
    ...
    def create_delete_imei_tab(self):
        ttk.Label(self.delete_imei_frame, text="Delete Trips by IMEI",
                  font=('Helvetica', 14)).pack(pady=10)
        ...
        self.preview_delete_imei_button = ttk.Button(
            self.delete_imei_frame, text="Preview Deletion", command=self.preview_delete_imei)
        self.preview_delete_imei_button.pack(pady=5)
        self.delete_imei_button = ttk.Button(
            self.delete_imei_frame, text="Delete", command=self.delete_by_imei)
        self.delete_imei_button.pack(pady=5)

        self.delete_imei_text = ScrolledText(
            self.delete_imei_frame, height=20, width=90)
    ...
    def preview_delete_imei(self):
        ...
        total_count = collection.count_documents(query)
        if samples:
            self.delete_imei_text.insert(
                tk.END, f"Total documents to delete: {total_count}\n\nSample documents:\n\n")
            for doc in samples:
                self.delete_imei_text.insert(tk.END, json.dumps(
                    doc, default=str, indent=4) + "\n\n")
        else:
            self.delete_imei_text.insert(
                tk.END, f"No documents found for IMEI '{imei}'.")
    ...
    def delete_by_imei(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to delete documents from '{collection_name}' with IMEI '{imei}'?")
        if confirm:
            threading.Thread(target=self._delete_by_imei_thread,
                             args=(collection, imei)).start()
    ...
    def create_remove_duplicates_tab(self):
        ttk.Label(self.remove_duplicates_frame, text="Remove Duplicate Trips", font=(
            'Helvetica', 14)).pack(pady=10)
        ttk.Label(self.remove_duplicates_frame,
                  text="Select Collection:").pack()
        ...
        self.preview_remove_duplicates_button = ttk.Button(
            self.remove_duplicates_frame, text="Preview Duplicates", command=self.preview_remove_duplicates)
        self.preview_remove_duplicates_button.pack(pady=5)
        self.remove_duplicates_button = ttk.Button(
            self.remove_duplicates_frame, text="Remove Duplicates", command=self.remove_duplicates)
        self.remove_duplicates_button.pack(pady=5)

        self.remove_duplicates_text = ScrolledText(
            self.remove_duplicates_frame, height=20, width=90)
    ...
    def preview_remove_duplicates(self):
        ...
        if duplicates:
            self.remove_duplicates_text.insert(
                tk.END, f"Total duplicate transactionIds: {total_duplicates}\n\nSample duplicates:\n\n")
            for dup in duplicates[:5]:
                self.remove_duplicates_text.insert(
                    tk.END, f"transactionId: {dup['_id']}, Count: {dup['count']}\n")
        else:
    ...
    def remove_duplicates(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to remove duplicates from '{collection_name}'?")
        if confirm:
            threading.Thread(target=self._remove_duplicates_thread,
                             args=(collection,)).start()
    ...
    def _remove_duplicates_thread(self, collection):
        try:
            ...
            messagebox.showinfo(
                "Success", f"Removed {total_removed} duplicate documents from '{collection.name}'.")
            self.view_collection_stats()
        except Exception as e:
            messagebox.showerror("Error", f"Error removing duplicates: {e}")

    def create_fix_missing_locations_tab(self):
        ttk.Label(self.fix_missing_locations_frame, text="Fix Missing Locations", font=(
            'Helvetica', 14)).pack(pady=10)
        ttk.Label(self.fix_missing_locations_frame,
                  text="Select Collection:").pack()
        ...
        self.preview_fix_missing_locations_button = ttk.Button(
            self.fix_missing_locations_frame, text="Preview Missing Locations", command=self.preview_fix_missing_locations)
        self.preview_fix_missing_locations_button.pack(pady=5)
        self.fix_missing_locations_button = ttk.Button(
            self.fix_missing_locations_frame, text="Fix Missing Locations", command=self.fix_missing_locations)
        self.fix_missing_locations_button.pack(pady=5)

        self.fix_missing_locations_text = ScrolledText(
            self.fix_missing_locations_frame, height=20, width=90)
    ...
    def preview_fix_missing_locations(self):
        ...
        total_count = collection.count_documents(query)
        if samples:
            self.fix_missing_locations_text.insert(
                tk.END, f"Total documents with missing locations: {total_count}\n\nSample documents:\n\n")
            for doc in samples:
                self.fix_missing_locations_text.insert(
                    tk.END, json.dumps(doc, default=str, indent=4) + "\n\n")
        else:
            self.fix_missing_locations_text.insert(
                tk.END, "No documents with missing locations found.")
    ...
    def fix_missing_locations(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to fix missing locations in '{collection_name}'?")
        if confirm:
            threading.Thread(
                target=self._fix_missing_locations_thread, args=(collection,)).start()
        ...
        try:
            ...
            for doc in cursor:
                try:
                    ...
                    print(
                        f"Error processing document {doc.get('transactionId', 'Unknown')}: {doc_error}")
                    continue

            messagebox.showinfo(
                "Success", f"Fixed missing locations for {updated_count} documents in '{collection.name}'.")
    ...
    def create_validate_geojson_tab(self):
        ttk.Label(self.validate_geojson_frame, text="Validate and Fix GeoJSON Data", font=(
            'Helvetica', 14)).pack(pady=10)
        ttk.Label(self.validate_geojson_frame,
                  text="Select Collection:").pack()
        ...
        self.preview_validate_geojson_button = ttk.Button(
            self.validate_geojson_frame, text="Preview Invalid GeoJSON", command=self.preview_validate_geojson)
        self.preview_validate_geojson_button.pack(pady=5)
        self.validate_geojson_button = ttk.Button(
            self.validate_geojson_frame, text="Validate and Fix GeoJSON", command=self.validate_geojson)
        self.validate_geojson_button.pack(pady=5)

        self.validate_geojson_text = ScrolledText(
            self.validate_geojson_frame, height=20, width=90)
    ...
    def preview_validate_geojson(self):
        ...
        if samples:
            self.validate_geojson_text.insert(
                tk.END, f"Sample invalid GeoJSON documents:\n\n")
            for doc in samples:
                self.validate_geojson_text.insert(
                    tk.END, json.dumps(doc, default=str, indent=4) + "\n\n")
        else:
            self.validate_geojson_text.insert(
                tk.END, "No invalid GeoJSON found in sample.")
    ...
    def validate_geojson(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to validate and fix GeoJSON in '{collection_name}'?")
        if confirm:
            threading.Thread(target=self._validate_geojson_thread,
                             args=(collection,)).start()
        ...
        try:
            ...
            for doc in cursor:
                ...
                try:
                    ...
                    fixed_count += 1
            messagebox.showinfo(
                "Success", f"Validated GeoJSON for '{collection.name}', fixed {fixed_count} documents.")
        except Exception as e:
            messagebox.showerror("Error", f"Error validating GeoJSON: {e}")

    def create_recalculate_distances_tab(self):
        ttk.Label(self.recalculate_distances_frame, text="Recalculate Distances", font=(
            'Helvetica', 14)).pack(pady=10)
        ttk.Label(self.recalculate_distances_frame,
                  text="Select Collection:").pack()
        ...
        self.preview_recalculate_distances_button = ttk.Button(
            self.recalculate_distances_frame, text="Preview Recalculations", command=self.preview_recalculate_distances)
        self.preview_recalculate_distances_button.pack(pady=5)
        self.recalculate_distances_button = ttk.Button(
            self.recalculate_distances_frame, text="Recalculate Distances", command=self.recalculate_distances)
        self.recalculate_distances_button.pack(pady=5)

        self.recalculate_distances_text = ScrolledText(
            self.recalculate_distances_frame, height=20, width=90)
    ...
    def preview_recalculate_distances(self):
        ...
        try:
            samples = list(collection.find().limit(5))
            if samples:
                self.recalculate_distances_text.insert(
                    tk.END, "Sample recalculations:\n\n")
                for doc in samples:
                    gps_data = doc['gps']
                    if isinstance(gps_data, str):
                        gps_data = json.loads(gps_data)
                    ...
                    self.recalculate_distances_text.insert(
                        tk.END, f"Original distance: {doc.get('distance', 'N/A')}, Recalculated: {round(distance, 2)} miles\n\n")
            else:
                self.recalculate_distances_text.insert(
                    tk.END, "No documents found.")
        except Exception as e:
    ...
    def recalculate_distances(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to recalculate distances in '{collection_name}'?")
        if confirm:
            threading.Thread(
                target=self._recalculate_distances_thread, args=(collection,)).start()
    ...
    def _recalculate_distances_thread(self, collection):
        try:
            ...
            for doc in cursor:
                ...
                updated_count += 1
            messagebox.showinfo(
                "Success", f"Recalculated distances for {updated_count} documents in '{collection.name}'.")
        except Exception as e:
            messagebox.showerror(
                "Error", f"Error recalculating distances: {e}")
    ...
    def create_update_timezones_tab(self):
        ttk.Label(self.update_timezones_frame, text="Update Timezones",
                  font=('Helvetica', 14)).pack(pady=10)
        ttk.Label(self.update_timezones_frame,
                  text="Select Collection:").pack()
        ...
        self.preview_update_timezones_button = ttk.Button(
            self.update_timezones_frame, text="Preview Timezones", command=self.preview_update_timezones)
        self.preview_update_timezones_button.pack(pady=5)
        self.update_timezones_button = ttk.Button(
            self.update_timezones_frame, text="Update Timezones", command=self.update_timezones)
        self.update_timezones_button.pack(pady=5)

        self.update_timezones_text = ScrolledText(
            self.update_timezones_frame, height=20, width=90)
    ...
    def preview_update_timezones(self):
        ...
        try:
            ...
            samples = list(collection.find().limit(5))
            if samples:
                self.update_timezones_text.insert(
                    tk.END, "Sample timezone updates:\n\n")
                for doc in samples:
                    gps_data = doc.get('gps')
                    if not gps_data:
                        continue
                    ...
                    self.update_timezones_text.insert(
                        tk.END, f"Original timezone: {original_timezone}, New timezone: {timezone}\n\n")
            else:
                self.update_timezones_text.insert(
                    tk.END, "No documents found.")
        except Exception as e:
    ...
    def update_timezones(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to update timezones in '{collection_name}'?")
        if confirm:
            threading.Thread(target=self._update_timezones_thread,
                             args=(collection,)).start()
        ...
        try:
            ...
            for doc in cursor:
                ...
                if timezone:
                    ...
                    updated_count += 1
            messagebox.showinfo(
                "Success", f"Updated timezones for {updated_count} documents in '{collection.name}'.")
        except Exception as e:
            messagebox.showerror("Error", f"Error updating timezones: {e}")

    def create_fix_time_tab(self):
        ttk.Label(self.fix_time_frame, text="Fix Time Offsets",
                  font=('Helvetica', 14)).pack(pady=10)
        ...
        ttk.Label(self.fix_time_frame,
                  text="Hours to Adjust (+/-):").pack(pady=5)
        self.fix_time_offset_entry = ttk.Entry(self.fix_time_frame)
        self.fix_time_offset_entry.pack()
        self.preview_fix_time_button = ttk.Button(
            self.fix_time_frame, text="Preview Changes", command=self.preview_fix_time_offsets)
        self.preview_fix_time_button.pack(pady=5)
        self.fix_time_button = ttk.Button(
            self.fix_time_frame, text="Apply Time Fix", command=self.fix_time_offsets)
        self.fix_time_button.pack(pady=5)
        self.fix_time_text = ScrolledText(
            self.fix_time_frame, height=20, width=90)
    ...
    def preview_fix_time_offsets(self):
        ...
        try:
            ...
            self.fix_time_text.insert(
                tk.END, "Please enter a valid number for hours offset.")
            return
        ...
        if samples:
            ...
            for doc in samples:
                ...
                corrected_start_time = original_start + \
                    timedelta(hours=hours_offset)
                corrected_end_time = original_end + \
                    timedelta(hours=hours_offset)
                self.fix_time_text.insert(
                    tk.END, f"Original Start: {original_start}, Adjusted Start: {corrected_start_time}\n")
                self.fix_time_text.insert(
                    tk.END, f"Original End: {original_end}, Adjusted End: {corrected_end_time}\n\n")
        else:
            self.fix_time_text.insert(
                tk.END, "No documents found in this collection.")
    ...
    def fix_time_offsets(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", "Please enter a valid number for hours offset.")
            return
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to adjust times by {hours_offset} hours in '{collection_name}'?")
        if confirm:
            threading.Thread(target=self._fix_time_offsets_thread,
                             args=(collection, hours_offset)).start()
    ...
    def _fix_time_offsets_thread(collection, hours_offset):
        try:
            ...
            updated_count = 0
            for trip in trips:
                corrected_start_time = trip['startTime'] + \
                    timedelta(hours=hours_offset)
                corrected_end_time = trip['endTime'] + \
                    timedelta(hours=hours_offset)
                collection.update_one(
                    {"_id": trip["_id"]},
                    {"$set": {
                ...
                updated_count += 1
            messagebox.showinfo(
                "Success", f"Updated {updated_count} documents in '{collection.name}'.")
        except Exception as e:
            messagebox.showerror("Error", f"Error fixing time offsets: {e}")

    def create_undo_time_tab(self):
        ttk.Label(self.undo_time_frame, text="Undo Time Fixes",
                  font=('Helvetica', 14)).pack(pady=10)
        ...
        ttk.Label(self.undo_time_frame,
                  text="Hours to Undo (+/-):").pack(pady=5)
        self.undo_time_offset_entry = ttk.Entry(self.undo_time_frame)
        self.undo_time_offset_entry.pack()
        self.preview_undo_time_button = ttk.Button(
            self.undo_time_frame, text="Preview Changes", command=self.preview_undo_time_offsets)
        self.preview_undo_time_button.pack(pady=5)
        self.undo_time_button = ttk.Button(
            self.undo_time_frame, text="Undo Time Fix", command=self.undo_time_offsets)
        self.undo_time_button.pack(pady=5)
        self.undo_time_text = ScrolledText(
            self.undo_time_frame, height=20, width=90)
    ...
    def undo_time_offsets(self):
        ...
        try:
            ...
            return
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to undo time adjustment by {hours_offset} hours in '{collection_name}'?")
        if confirm:
            threading.Thread(target=self._undo_time_offsets_thread,
                             args=(collection, hours_offset)).start()
    ...
    def create_backup_tab(self):
        ttk.Label(self.backup_frame, text="Select a collection to backup:", font=(
            'Helvetica', 14)).pack(pady=10)
        ...
        self.preview_backup_button = ttk.Button(
            self.backup_frame, text="Preview Backup Data", command=self.preview_backup)
        self.preview_backup_button.pack(pady=5)
        self.backup_button = ttk.Button(
            self.backup_frame, text="Backup Collection", command=self.backup_collection)
    ...
    def preview_backup(self):
        ...
        try:
            samples = list(collection.find().limit(5))
            if samples:
                self.backup_text.insert(
                    tk.END, f"Sample data from '{collection_name}':\n\n")
                for doc in samples:
                    self.backup_text.insert(tk.END, json.dumps(
                        doc, default=str, indent=4) + "\n\n")
            else:
                self.backup_text.insert(
                    tk.END, "No documents found in this collection.")
    ...
    def backup_collection(self):
        ...
        save_path = filedialog.asksaveasfilename(
            defaultextension=".json", initialfile=f"{collection_name}_backup.json")
        if save_path:
            threading.Thread(target=self._backup_collection_thread,
                             args=(collection, save_path)).start()
    ...
    def _backup_collection_thread(collection, save_path):
        try:
            ...
            if not documents:
                messagebox.showinfo(
                    "Info", f"No documents found in the '{collection.name}' collection.")
                return
            with open(save_path, 'w') as backup_file:
                json.dump(documents, backup_file, default=json_util.default)
            messagebox.showinfo(
                "Success", f"Backup of '{collection.name}' saved to '{save_path}'.")
        except Exception as e:
            messagebox.showerror("Error", f"Error backing up collection: {e}")

    def create_restore_tab(self):
        ttk.Label(self.restore_frame, text="Select a collection to restore:", font=(
            'Helvetica', 14)).pack(pady=10)
        ...
        self.load_backup_button = ttk.Button(
            self.restore_frame, text="Load Backup File", command=self.load_backup_file)
        self.load_backup_button.pack(pady=5)
        self.preview_restore_button = ttk.Button(
            self.restore_frame, text="Preview Restore Data", command=self.preview_restore)
        self.preview_restore_button.pack(pady=5)
        self.restore_button = ttk.Button(
            self.restore_frame, text="Restore Collection", command=self.restore_collection)
        self.restore_button.pack(pady=5)
        self.restore_text = ScrolledText(
            self.restore_frame, height=20, width=90)
        ...
        if file_path:
            try:
                with open(file_path, 'r') as backup_file:
                    self.backup_data = json.load(
                        backup_file, object_hook=json_util.object_hook)
                self.restore_text.delete(1.0, tk.END)
                self.restore_text.insert(
                    tk.END, f"Loaded backup file: {file_path}\n")
                self.restore_text.insert(
                    tk.END, "You can now preview or restore the data.")
            except Exception as e:
                messagebox.showerror(
                    "Error", f"Error loading backup file: {e}")
    ...
    def preview_restore(self):
        ...
        try:
            ...
            if samples:
                ...
                for doc in samples:
                    self.restore_text.insert(tk.END, json.dumps(
                        doc, default=str, indent=4) + "\n\n")
            else:
    ...
    def restore_collection(self):
        ...
        collection = collections[collection_name]
        confirm = messagebox.askyesno(
            "Confirm", f"Are you sure you want to restore the '{collection_name}' collection?\nThis will overwrite existing data.")
        if confirm:
            threading.Thread(
                target=self._restore_collection_thread, args=(collection,)).start()
    ...
    def _restore_collection_thread(self, collection):
        try:
            ...
            if self.backup_data:
                ...
                messagebox.showinfo(
                    "Success", f"Restored {len(self.backup_data)} documents to '{collection.name}'.")
                self.view_collection_stats()
            else:
    ...
    def create_places_tab(self):
        ttk.Label(self.places_frame, text="Manage Places Collection",
                  font=('Helvetica', 14)).pack(pady=10)

        # Add buttons for common places operations
        ttk.Button(self.places_frame, text="View Places",
                   command=self.view_places).pack(pady=5)
        ttk.Button(self.places_frame, text="Validate Place Geometries",
                   command=self.validate_place_geometries).pack(pady=5)
        ttk.Button(self.places_frame, text="Recalculate Visit Statistics",
                   command=self.recalculate_visit_stats).pack(pady=5)
    ...
    def view_places(self):
        ...
        try:
            ...
            if places:
                for place in places:
                    self.places_text.insert(tk.END, json.dumps(
                        place, default=str, indent=4) + "\n\n")
            else:
                self.places_text.insert(
                    tk.END, "No places found in the collection.")
        except Exception as e:
    ...
    def validate_place_geometries(self):
        try:
            ...
            if invalid_places:
                self.places_text.delete(1.0, tk.END)
                self.places_text.insert(
                    tk.END, f"Found {len(invalid_places)} invalid place geometries:\n")
                for place_id in invalid_places:
                    self.places_text.insert(tk.END, f"Place ID: {place_id}\n")
            else:
                messagebox.showinfo(
                    "Success", "All place geometries are valid.")
        except Exception as e:
            messagebox.showerror(
                "Error", f"Error validating place geometries: {e}")
    ...
    def recalculate_visit_stats():
        try:
            ...
            messagebox.showinfo(
                "Success", "Visit statistics have been recalculated for all places.")
        except Exception as e:
            messagebox.showerror(
                "Error", f"Error recalculating visit statistics: {e}")
