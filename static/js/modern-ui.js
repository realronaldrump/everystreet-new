...
const ModernUI = {
  ...
  initFloatingActionButton() {
    ...
    document.addEventListener('click', (e) => {
      if (!actionButton.contains(e.target) && !actionMenu.contains(e.target) && actionMenu.classList.contains('open')) {
        actionMenu.classList.remove('open');
        actionButton.classList.remove('active');
  ...
  handleFetchTrips() {
    ...
    const opId = window.loadingManager ? 
      window.loadingManager.startOperation('fetchTrips', 100) : null;
    ...
    .then(data => {
      ...
      } else if (window.fetchTrips && typeof window.fetchTrips === 'function') {
  ...
  handleMapMatch() {
    ...
    // Track the loading operation
    const opId = window.loadingManager ? 
      window.loadingManager.startOperation('mapMatching', 100) : null;
  ...
  submitPlace() {
    ...
    if (window.bootstrap?.Modal) {
      const modal = bootstrap.Modal.getInstance(document.getElementById('add-place-modal'));
      if (modal) modal.hide();
    }
    ...
    document.addEventListener('filtersApplied', (e) => {
      ...
      if (typeof fetchTrips === 'function') {
        ...
        if (window.fetchTrips) {
          ...
          const applyFiltersBtn = document.getElementById('apply-filters');
          if (applyFiltersBtn && applyFiltersBtn !== document.querySelector('.filters-panel #apply-filters')) {
...
window.loadingManager = {
  ...
  addSubOperation: (parentOperation, subOperationName, weight = 1) => {
    ...
    console.log(`Added sub-operation ${subOperationName} to ${parentOperation}`);
  },
  
  // Update a sub-operation's progress
  updateSubOperation: (parentOperation, subOperationName, progress, message) => {
    const operation = window.loadingManager.operations.get(parentOperation);
