...
(() => {
    ...
    class VisitsManager {
            ...
            { data: 'totalVisits' },
            {
              ...
              render: (data, type) =>
                (type === 'display' || type === 'filter')
                  ? (data ? new Date(data).toLocaleDateString('en-US') : 'N/A')
                  : data,
            },
            {
              data: 'lastVisit',
      ...
      setupEventListeners() {
        ...
        $('#visits-table-container').on('click', '#toggle-view-btn', () => this.toggleView());
      }
      ...
      async updateVisitsData() {
        ...
        for (const [id, place] of this.places) {
          try {
            ...
            if (!response.ok) throw new Error(`Failed to fetch statistics for place ${id}`);
      ...
      async savePlace() {
        ...
        const placeData = { name: placeName, geometry: this.currentPolygon.toGeoJSON().geometry };
        try {
          const response = await fetch('/api/places', {
            method: 'POST',
      ...
      async loadNonCustomPlacesVisits() {
        try {
          ...
          if (!response.ok) throw new Error('Failed to fetch visits for non-custom places');
