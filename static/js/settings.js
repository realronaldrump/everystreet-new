...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const mobileAutoCenterToggle = document.getElementById(
    "mobile-auto-center-toggle",
  );
  const mobileShowLiveTracking = document.getElementById(
    "mobile-show-live-tracking",
  );
  const mobilePolylineColor = document.getElementById("mobile-polyline-color");
  const mobilePolylineOpacity = document.getElementById(
    "mobile-polyline-opacity",
  );
  ...
  function applySettings(settings = {}) {
    ...
    if (mobileHighlightRecentTrips)
      mobileHighlightRecentTrips.checked = hrt !== false;
    if (mobileAutoCenterToggle)
      mobileAutoCenterToggle.checked = autoCenter !== false;
    if (mobileShowLiveTracking) mobileShowLiveTracking.checked = slt !== false;
    if (mobileGeocodeTripsOnFetch)
      mobileGeocodeTripsOnFetch.checked = gtof !== false;
