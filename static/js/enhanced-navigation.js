    ...
    try {
      ...
      if (
        ...
        try {
          ...
          if (
            this.undrivenStreetsLayer.getLayers().length > 0 &&
            typeof this.undrivenStreetsLayer.getBounds === "function"
          ) {
            ...
            console.warn(
              "No layers added to undrivenStreetsLayer or getBounds is not a function. Cannot get bounds.",
            );
          }
        } catch (e) {
  ...
  async findAndDisplayRoute() {
    ...
    try {
      ...
      try {
        ...
        if (
          this.positionMarker &&
          typeof this.positionMarker.getLatLng === "function"
        ) {
        ...
        if (
          boundsToFit &&
          typeof boundsToFit.isValid === "function" &&
          boundsToFit.isValid()
        ) {
          this.map.fitBounds(boundsToFit, { padding: [70, 70] });
        } else {
          console.warn(
            "Cannot fit map to route and user location: bounds are not valid.",
          );
        }
      } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          console.warn(
            "Cannot fit map to segment route and user location: bounds are not valid.",
          );
        }
      } catch (error) {
        console.error("Error fitting map bounds:", error);
