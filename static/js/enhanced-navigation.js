...
class EnhancedNavigation {
  constructor() {
    ...
    this.directionInstruction = document.getElementById("direction-instruction");
  ...
  startLocationTracking() {
    ...
    if (success) {
      ...
      this.setStatus("Unable to access your location. Navigation features may be limited.", true);
    }
  }
  ...
  setupEventListeners() {
    ...
    if (this.calcCoverageBtn) {
      this.calcCoverageBtn.addEventListener("click", () => this.calculateCoverageRoute());
    }
    
    // Center position button
    if (this.centerPositionBtn) {
      this.centerPositionBtn.addEventListener("click", () => this.centerOnUserLocation());
    }
    
    // Stop navigation button
    if (this.stopNavigationBtn) {
      this.stopNavigationBtn.addEventListener("click", () => this.stopNavigation());
    }
  ...
  async handleAreaChange() {
    ...
    const selectedArea = this.coverageAreas.find(area => 
      area.location?.display_name === selectedAreaName
    );
  ...
  async fetchAndDisplayUndrivenStreets() {
    if (!this.selectedLocation || !this.selectedLocation.location) {
      this.setStatus("Please select an area first or selected area has no location data.", true);
      return;
    }
    ...
    console.log("Request body for /api/undriven_streets:", JSON.stringify(requestPayload, null, 2));
    console.log("Value of boundary_geojson being sent as 'boundary':", locationData.boundary_geojson);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
            ...
            console.error("Server Error Data for /api/undriven_streets:", errorData); 
            ...
            console.error("Could not parse error response as JSON or get text:", e);
      ...
      if (data && data.features && Array.isArray(data.features) && data.features.length > 0) {
        try {
          L.geoJSON(data, {
            ...
            onEachFeature: (feature, layer) => {
              if (feature.properties && feature.properties.segment_id) {
                ...
                const streetName = feature.properties.street_name || "Unnamed Street";
                layer.bindPopup(`
                  <div>
          ...
          this.setStatus(`Error displaying undriven streets on map: ${e.message}`, true);
        }
        
        this.setStatus(`Loaded ${data.features.length} undriven street segments.`);
      } else {
        console.warn("No features found in /api/undriven_streets response or data is malformed. Data:", data);
        this.setStatus("No undriven streets found in this area or data format issue.");
      }
    } catch (error) {
  ...
  async findAndDisplayRoute() {
    ...
    this.findBtn.disabled = true;
    this.findBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Finding Route...';
    ...
    try {
      ...
      if (!route) {
        ...
        this.findBtn.disabled = false;
        this.findBtn.innerHTML = '<i class="fas fa-route me-2"></i>Find Nearest Undriven Street';
        this.isFetchingRoute = false;
        return;
      }
  ...
  highlightTargetStreet(targetStreet) {
    ...
    const startCoords = [targetStreet.start_coords[1], targetStreet.start_coords[0]];
    ...
    L.marker(startCoords, { icon: destinationIcon })
      .addTo(this.targetStreetLayer)
      .bindPopup(`
  ...
  async calculateCoverageRoute() {
    ...
    this.setStatus("Full coverage route calculation is not implemented in this version");
  }
  ...
  recalculateRoute() {
    if (!this.isNavigating || !this.selectedLocation || !this.selectedLocation.location) return;
    ...
    this.navigationService.requestRoute(this.selectedLocation.location, options);
  }
    ...
    switch (update.type) {
      ...
      case "progress":
        ...
        if (update.remainingDistance && update.remainingDuration) {
          const distanceMiles = (update.remainingDistance * 0.000621371).toFixed(1);
          ...
          if (this.navigationService.currentRoute) {
            ...
            const progress = 100 - (update.remainingDistance / totalDistance * 100);
  ...
  resetSteps() {
    [this.stepClustering, this.stepOptimizing, this.stepRendering].forEach(step => {
      if (step) {
  ...
  async findRouteToSegment(segmentId) {
    if (!this.selectedLocation || !this.selectedLocation.location || !segmentId) return;
