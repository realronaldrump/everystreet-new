...
(() => {
  ...
  async function downloadSelectedRegion() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          responseErrorMessage(response, data, "Failed to start download"),
        );
      }
      if (!data || !data.job_id) {
  ...
  async function autoDetectRegion() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          responseErrorMessage(response, data, "No region suggestion found"),
        );
      }
  ...
  function startRegionJob(jobId, name) {
    ...
    regionPollInterval = setInterval(async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            responseErrorMessage(response, data, "Failed to read job status"),
          );
        }
        if (!data) {
          throw new Error("Unexpected response from server.");
