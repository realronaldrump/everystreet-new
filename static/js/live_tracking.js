    ...
    window.LiveTripTracker = class LiveTripTracker {
        ...
        connectToSocket() {
            try {
                ...
                this.updateConnectionStatus(false); // Indicate connection failure
            }
        }
        ...
        initializeTrip(tripData) {
            ...
            const trip = {
                polyline: L.polyline([], { color: '#FF5722', weight: 4, opacity: 0.8, className: 'active-trip' }).addTo(this.liveTripsLayer),
                coordinates: [],
        ...
        updateTripPath(tripData) {
            ...
            tripData.data.forEach(point => {
                if (point.gps?.lat && point.gps?.lon) { // Optional chaining
                    ...
                    trip.dataPoints.push({ timestamp: new Date(point.timestamp), speed: point.speed || 0, coordinate: coord });
                }
            });
            ...
            if (trip.coordinates.length > 0) {
                this.updateVehicleMarker(tripId, trip.coordinates.at(-1)); // Use .at(-1) for last element
        ...
        updateVehicleMarker(tripId, position) {
            ...
            if (!trip.vehicleMarker) {
                const icon = L.divIcon({ className: 'vehicle-marker', html: '<i class="fas fa-car"></i>', iconSize: [24, 24], iconAnchor: [12, 12] });
        ...
        finalizeTrip(tripData) {
            ...
            if (trip.coordinates.length > 0) {
                const endPoint = trip.coordinates.at(-1);  // Use .at(-1)
                L.marker(endPoint, { icon: L.divIcon({ className: 'trip-marker trip-end', html: '<i class="fas fa-flag-checkered"></i>' }) }).addTo(this.liveTripsLayer);
            }

            trip.vehicleMarker && this.liveTripsLayer.removeLayer(trip.vehicleMarker); // Short-circuit evaluation
        ...
        clearCompletedTrips() {
            ...
            for (const [tripId, trip] of this.activeTrips) {
                const lastUpdate = trip.dataPoints.length > 0 ? trip.dataPoints.at(-1).timestamp : trip.startTime;
