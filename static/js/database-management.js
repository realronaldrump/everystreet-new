...
document.addEventListener('DOMContentLoaded', () => {
      ...
      if (isLoading) {
        ...
        switch (action) {
          ...
          case 'repair-indexes':
            ...
            break;
          case 'optimize':
            button.innerHTML = '<i class="fas fa-compress-arrows-alt"></i> Optimize';
            break;
          case 'clear':
    ...
    function updateStorageDisplay(data) {
      if (progressBar) {
        ...
        progressBar.classList.toggle('bg-warning', data.usage_percent > 80 && data.usage_percent <= 95);
    ...
    optimizeAllBtn?.addEventListener('click', async () => {
      ...
      const confirmed = await confirmationDialog.show({
        message: 'Are you sure you want to optimize all collections? This may take a while.',
      });
       if (confirmed) {
            handleConfirmedAction();
        }
