...
class ModalManager {
    ...
    // Progress modal
    showProgressModal(title, initialMessage = 'Processing...', initialProgress = 0) {
    ...
    updateProgressModal(data) {
      ...
      if (cancelBtn) {
        cancelBtn.disabled = ['complete', 'completed', 'error', 'canceled'].includes(stage);
      }
    }
    ...
    hideProgressModal(modalId = 'taskProgressModal') {
      ...
      const bsModal = this.activeModals.get(modalId) || bootstrap.Modal.getInstance(modal);
      if (bsModal) {
        bsModal.hide();
    ...
    createBatchProgressModal(total) {
      ...
      return {
        ...
        updateProgress: (current, completed, failed) => {
          ...
          this.ui.updateProgress(progressBar, percentage, `${current} / ${total}`);
    ...
    formatMetricStats(stage, metrics) {
      ...
      if (metrics.total_length_m !== undefined) {
        addStat('Total Length', this.ui.constructor.distanceInUserUnits(metrics.total_length_m), '', 'fas fa-ruler-horizontal');
      }
      if (metrics.processed_trips !== undefined && metrics.total_trips_to_process !== undefined) {
        const progress = ((metrics.processed_trips / metrics.total_trips_to_process) * 100).toFixed(1);
        addStat('Trips Progress', `${progress}%`, '', 'fas fa-route', 'text-info');
      }
      if (metrics.coverage_percentage !== undefined) {
        addStat('Coverage', metrics.coverage_percentage.toFixed(1), '%', 'fas fa-tachometer-alt', 'text-success');
      }
