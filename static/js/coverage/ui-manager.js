...
class UIManager {
    ...
    setValidationState(element, isValid, message = '') {
      ...
      if (message) {
        ...
        if (!feedback) {
          feedback = this.createElement('div', isValid ? 'valid-feedback' : 'invalid-feedback');
    ...
    updateTable(tableId, data, rowRenderer) {
      ...
      if (!data.length) {
        tbody.innerHTML = this.createEmptyTableRow(table.querySelectorAll('thead th').length);
        return;
      }
    ...
    updateStats(container, stats) {
      ...
      const html = Object.entries(stats).map(([key, { value, label, className }]) => 
    ...
    createAlert(type, title, message, dismissible = true) {
      ...
      return this.createElement('div', `alert alert-${type} ${dismissible ? 'alert-dismissible' : ''} fade show`, `
    ...
    updateFilterButtons(buttons, activeFilter) {
      buttons.forEach(btn => {
        ...
        btn.classList.add(isActive ? baseClass : `btn-outline-${baseClass.split('-')[1]}`);
