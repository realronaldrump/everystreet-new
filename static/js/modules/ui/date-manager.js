...
const dateManager = {
  ...
  async setRange(range) {
    if (!DateUtils) { utils.showNotification('Date utility missing', 'danger'); return; }
    ...
    try {
      ...
      if (startDate && endDate) {
        ...
        this.updateIndicator();
        uiState.getAllElements('.quick-select-btn').forEach((b)=>b.classList.toggle(CONFIG.classes.active,b.dataset.range===range));
  ...
  detectPreset(start, end) {
    ...
    if (s.toDateString() === e.toDateString() && s.toDateString() === today.toDateString()) return 'today';
    const yest = new Date(today); yest.setDate(yest.getDate()-1);
    if (s.toDateString() === yest.toDateString() && e.toDateString() === yest.toDateString()) return 'yesterday';
  ...
  updateIndicator() {
    ...
    if (preset) {
      span.textContent = preset.charAt(0).toUpperCase()+preset.slice(1).replace('-',' ');
  ...
  async applyFilters() {
    ...
    if (!sIn || !eIn) { utils.showNotification('Date input elements missing','danger'); return; }
    const s = sIn.value; const e = eIn.value;
    if (!DateUtils?.isValidDateRange?.(s,e)) {
      utils.showNotification('Invalid date range','warning');
      [sIn,eIn].forEach(el=>{ el.classList.add('invalid-shake'); setTimeout(()=>el.classList.remove('invalid-shake'),600); });
      return;
    }
    ...
    try {
      ...
      document.dispatchEvent(new CustomEvent('filtersApplied',{detail:{startDate:s,endDate:e}}));
  ...
  reset() {
    ...
    uiState.getAllElements('.quick-select-btn').forEach(btn=>btn.classList.remove(CONFIG.classes.active));
