...
if (!utils.fadeIn) {
  utils.fadeIn = (el, duration = 200) => new Promise((resolve) => {
...
if (!utils.fadeOut) {
  utils.fadeOut = (el, duration = 200) => new Promise((resolve) => {
    if (!el) return resolve();
    el.style.opacity = 1;
    el.style.transition = `opacity ${duration}ms`;
...
if (!utils.measureScrollbarWidth) {
  utils.measureScrollbarWidth = () => window.innerWidth - document.documentElement.clientWidth;
}
