  ...
  const Dashboard = {
    async displayCoverageDashboard(manager, locationId) {
      ...
      if (!dashboardElement || !locationNameElement || !mapContainer) {
        manager.notificationManager?.show("UI Error: Dashboard components missing.", "danger");
        return;
      }
      ...
      if (streetTypeChartElement) streetTypeChartElement.innerHTML = Dashboard.createLoadingSkeleton(180);
      if (streetTypeCoverageElement) streetTypeCoverageElement.innerHTML = Dashboard.createLoadingSkeleton(100, 3);
      mapContainer.innerHTML = Dashboard.createLoadingIndicator("Loading map data...");
      try {
        const cachedData = manager.getCachedData?.(`dashboard-${locationId}`);
        let coverageData;
        if (cachedData) {
          coverageData = cachedData;
          manager.notificationManager?.show("Loaded dashboard from cache.", "info", 1500);
        } else {
          ...
          const streetsResp = await fetch(`/api/coverage_areas/${locationId}/streets?cache_bust=${Date.now()}`);
          if (!streetsResp.ok) {
        ...
        Dashboard.updateStreetTypeCoverage(manager, coverageData.street_types || []);
        Dashboard.createStreetTypeChart(manager, coverageData.street_types || []);
        manager.updateFilterButtonStates?.();
        Dashboard.initializeCoverageMap(manager, coverageData);
        manager.showTripsActive = localStorage.getItem("showTripsOverlay") === "true";
        ...
        manager.notificationManager?.show(`Error loading dashboard: ${error.message}`, "danger");
        mapContainer.innerHTML = Dashboard.createAlertMessage("Dashboard Load Error", error.message, "danger", locationId);
      } finally {
    ...
    createLoadingSkeleton(height, count = 1) {
      return Array.from({ length: count })
        .map(() => `<div class="loading-skeleton skeleton-shimmer mb-2" style="height: ${height}px;"></div>`) 
        .join("");
    },
    ...
    updateDashboardStats(manager, coverage) {
      ...
      if (Array.isArray(coverage.street_types)) {
        coveredSegments = coverage.street_types.reduce((sum, t) => sum + (parseInt(t.covered, 10) || parseInt(t.covered_segments, 10) || 0), 0);
      }
      const lastUpdated = coverage.last_stats_update || coverage.last_updated
        ? manager.formatRelativeTime?.(coverage.last_stats_update || coverage.last_updated)
        : "Never";
      let barColor = "bg-success";
      if ([STATUS.ERROR, STATUS.CANCELED].includes(coverage.status)) barColor = "bg-danger";
      else if (![STATUS.COMPLETED, STATUS.COMPLETE].includes(coverage.status)) barColor = "bg-warning";
      const html = `
        <div class="row g-3">
    ...
    updateStreetTypeCoverage(manager, streetTypes) {
      ...
      if (!streetTypes || !streetTypes.length) {
        streetTypeCoverageEl.innerHTML = Dashboard.createAlertMessage("No Data", "No street type data available.", "secondary");
        return;
      }
      const sorted = [...streetTypes].sort((a, b) => parseFloat(b.total_length_m || 0) - parseFloat(a.total_length_m || 0));
      ...
      streetTypeCoverageEl.innerHTML = topTypes
        .map((type) => {
          ...
          const coveredDist = CoverageShared.UI.distanceInUserUnits(parseFloat(type.covered_length_m || 0));
          const totalDist = CoverageShared.UI.distanceInUserUnits(parseFloat((type.driveable_length_m ?? type.total_length_m) || 0));
          const denominatorLabel = type.driveable_length_m !== undefined ? "Driveable" : "Total";
          const barColor = parseFloat(coveragePct) < 25 ? "bg-danger" : parseFloat(coveragePct) < 75 ? "bg-warning" : "bg-success";
          return `
    ...
    clearDashboardUI(manager) {
      ...
      if (manager.coverageMap) {
        try { manager.coverageMap.remove(); } catch (_) {}
        manager.coverageMap = null;
      }
      manager.selectedLocation = null;
      manager.streetsGeoJson = null;
      manager.mapBounds = null;
      if (manager.streetTypeChartInstance) { manager.streetTypeChartInstance.destroy(); manager.streetTypeChartInstance = null; }
      manager.currentDashboardLocationId = null;
      if (manager.mapInfoPanel) { manager.mapInfoPanel.remove(); manager.mapInfoPanel = null; }
      if (manager.coverageSummaryControl && manager.coverageMap) {
        try { manager.coverageMap.removeControl(manager.coverageSummaryControl); } catch (_) {}
    ...
    createAlertMessage(title, message, type = "info", locationId = null) {
      const icon = { danger: "fa-exclamation-circle", warning: "fa-exclamation-triangle", info: "fa-info-circle", secondary: "fa-question-circle" }[type] || "fa-info-circle";
    ...
    initializeCoverageMap(manager, coverage) {
      ...
      if (manager.coverageMap && typeof manager.coverageMap.remove === "function") {
        try { manager.coverageMap.remove(); } catch (e) { /* noop */ }
        manager.coverageMap = null;
      }
      mapContainer.innerHTML = "";
      if (!window.MAPBOX_ACCESS_TOKEN) {
        mapContainer.innerHTML = Dashboard.createAlertMessage("Mapbox Token Missing", "Cannot display map. Please configure Mapbox access token.", "danger");
        return;
      }
      mapboxgl.accessToken = window.MAPBOX_ACCESS_TOKEN;
      try {
        manager.coverageMap = new mapboxgl.Map({ container: "coverage-map", style: "mapbox://styles/mapbox/dark-v11", center: [0, 0], zoom: 1, minZoom: 0, maxZoom: 20, preserveDrawingBuffer: true, attributionControl: false });
        manager.coverageMap.addControl(new mapboxgl.NavigationControl(), "top-right");
        manager.coverageMap.addControl(new mapboxgl.ScaleControl());
        manager.coverageMap.addControl(new mapboxgl.FullscreenControl());
        manager.coverageMap.addControl(new mapboxgl.AttributionControl({ compact: true }), "bottom-right");
        manager.coverageMap.on("load", () => {
          if (coverage.streets_geojson) Dashboard.addStreetsToMap(manager, coverage.streets_geojson);
          else manager.notificationManager?.show("No street data found for this area.", "warning");
          Dashboard.addCoverageSummary?.(manager, coverage);
          Dashboard.fitMapToBounds(manager);
          manager.setupMapEventHandlers?.();
          if (manager.showTripsActive) { manager.setupTripLayers?.(); manager.loadTripsForView?.(); }
        });
        manager.coverageMap.on("error", (e) => {
          // eslint-disable-next-line no-console
          console.error("Mapbox GL Error:", e.error);
          manager.notificationManager?.show(`Map error: ${e.error?.message || "Unknown map error"}`, "danger");
          mapContainer.innerHTML = Dashboard.createAlertMessage("Map Load Error", e.error?.message || "Could not initialize map.", "danger");
        });
        ...
        mapContainer.innerHTML = Dashboard.createAlertMessage("Map Initialization Failed", err.message, "danger");
      }
    },

    addStreetsToMap(manager, geojson) {
      if (!manager.coverageMap || !manager.coverageMap.isStyleLoaded() || !geojson) return;
      ["streets-layer", "streets-hover-highlight", "streets-click-highlight", "streets-selection-highlight"].forEach((id) => {
        if (manager.coverageMap.getLayer(id)) manager.coverageMap.removeLayer(id);
      });
      if (manager.coverageMap.getSource("streets")) manager.coverageMap.removeSource("streets");
      manager.streetsGeoJson = geojson;
      manager.currentFilter = "all";
      manager.coverageMap.addSource("streets", { type: "geojson", data: geojson, promoteId: "segment_id" });
      const getLineColor = [
        "case",
      ...
      manager.coverageMap.addLayer({ id: "streets-layer", type: "line", source: "streets", layout: { "line-cap": "round", "line-join": "round" }, paint: { "line-color": getLineColor, "line-width": getLineWidth, "line-opacity": getLineOpacity, "line-dasharray": getLineDash } });
      const bounds = new mapboxgl.LngLatBounds();
      geojson.features.forEach((f) => {
        if (f.geometry?.coordinates) {
          if (f.geometry.type === "LineString") f.geometry.coordinates.forEach((c) => bounds.extend(c));
          else if (f.geometry.type === "MultiLineString") f.geometry.coordinates.forEach((line) => line.forEach((c) => bounds.extend(c)));
        }
      });
      ...
      manager.coverageMap.on("mouseenter", "streets-layer", (e) => {
        ...
        if (currentHoverId !== hoveredSegmentId) {
          if (hoveredSegmentId !== null && manager.coverageMap.getSource("streets")) {
            manager.coverageMap.setFeatureState({ source: "streets", id: hoveredSegmentId }, { hover: false });
          }
          if (manager.coverageMap.getSource("streets")) {
            manager.coverageMap.setFeatureState({ source: "streets", id: currentHoverId }, { hover: true });
          }
      ...
      manager.coverageMap.on("click", "streets-layer", (e) => {
        ...
        if (isMultiSelect) { const segId = props.segment_id; if (segId) manager.toggleSegmentSelection?.(segId); return; }
        ...
        popupElement?.addEventListener("click", (evt) => {
          ...
          if (action && segmentId) { manager._handleMarkSegmentAction?.(action, segmentId); popup.remove(); }
        });
    ...
    fitMapToBounds(manager) {
      if (manager.coverageMap && manager.mapBounds && !manager.mapBounds.isEmpty()) {
        try {
          manager.coverageMap.fitBounds(manager.mapBounds, { padding: 20, maxZoom: 17, duration: 800 });
        } catch (_) {
          manager.notificationManager?.show("Could not zoom to area bounds. Map view may be incorrect.", "warning");
        }
      } else if (manager.coverageMap) {
        manager.notificationManager?.show("No geographical data to display for this area.", "info");
      }
    },

    // (helpers already defined above)
    formatStreetType(type) { return !type ? "Unknown" : type.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()); },
    createStreetTypeChart(manager, streetTypes) {
      const chartContainer = document.getElementById("street-type-chart");
      if (!chartContainer) return;
      const labels = streetTypes.map((t) => Dashboard.formatStreetType(t.type));
      const dataValues = streetTypes.map((t) => parseFloat(t.coverage_percentage || 0).toFixed(1));
      if (manager.streetTypeChartInstance) manager.streetTypeChartInstance.destroy();
