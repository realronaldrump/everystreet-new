(function () {
  ...
  function extractGeometry(trip) {
    ...
    if (trip.startGeoPoint?.coordinates && trip.destinationGeoPoint?.coordinates) {
      return { type: 'LineString', coordinates: [trip.startGeoPoint.coordinates, trip.destinationGeoPoint.coordinates] };
    }
    return null;
  }
  ...
  function updateInfo(trip) {
    ...
    const dist = trip.distance ? parseFloat(trip.distance).toFixed(2) + ' miles' : 'Unknown';
...
<p><strong>Distance:</strong> ${dist}</p>`;
  ...
  function showTripOnMap(map, layerGroup, geometry) {
    ...
    const path = L.geoJSON(geometry, { style: { color: '#BB86FC', weight: 4, opacity: 0.8 } });
    layerGroup.addLayer(path);
    const coords = geometry.coordinates;
    L.marker([coords[0][1], coords[0][0]], { icon: L.divIcon({className:'trip-marker start-marker', html:'<i class="fas fa-play-circle"></i>', iconSize:[20,20], iconAnchor:[10,10]})}).addTo(layerGroup);
    L.marker([coords[coords.length-1][1], coords[coords.length-1][0]], { icon: L.divIcon({className:'trip-marker end-marker', html:'<i class="fas fa-stop-circle"></i>', iconSize:[20,20], iconAnchor:[10,10]})}).addTo(layerGroup);
