...
// Set up event listeners only when "Apply Filters" is pressed
  ...
  distanceChart = new Chart(distanceCtx, {
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          beginAtZero: true,
          title: { display: true, text: 'Distance (miles)', color: '#bb86fc' },
          ticks: { color: '#bb86fc' },
          grid: { color: 'rgba(187, 134, 252, 0.2)' }
        }
      },
...
function initializeDataTable() {
  // Create the DataTable with three columns:
  // Destination, Visit Count, and Last Visit.
...
function getFilterParams() {
  // Use the global date values stored in localStorage (set by your sidebar)
...
async function fetchDrivingInsights() {
  ...
  try {
    ...
    // Fetch insights and analytics concurrently
...
function updateDataTable(data) {
  ...
  // Convert the most_visited object into an object for the table with the key "destination"
  const visitedPlace = {
    destination: data.most_visited._id, // use this as the destination name
