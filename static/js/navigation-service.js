...
class NavigationService {
  ...
  initializeSpeech() {
    ...
    const getVoices = () => {
      ...
      const englishVoices = this.availableVoices.filter(
        (voice) => voice.lang.includes("en-") || voice.lang === "en",
      );
      ...
      console.log(
        "Speech synthesis initialized with",
  ...
  async requestRoute(locationObject, options = {}) {
    ...
    if (
      ...
      this._handleError(
        "Cannot request route: incomplete location data provided to NavigationService.",
      );
      console.error(
        "Incomplete locationObject in NavigationService:",
        locationObject,
      );
      return null;
    }
  ...
  startNavigation() {
    ...
    this._speak(
      `Starting navigation to ${this.targetStreet.street_name || "your destination"}`,
    );
  ...
  markStreetVisited(segmentId) {
    ...
    if (
      this.isNavigating &&
      this.targetStreet &&
      this.targetStreet.segment_id === segmentId
    ) {
      // Announce arrival
  ...
  async _processNextDirection() {
    if (
      this.processingDirections ||
      !this.isNavigating ||
      this.directionsQueue.length === 0
    ) {
      return;
    }
  ...
  _updateNavigationStatus() {
    ...
    this.currentRoute.remainingDuration =
      (distanceToTarget / this.currentRoute.distance) *
      this.currentRoute.duration;

    // Check if we've arrived at target
    if (distanceToTarget < 30) {
      // Within 30 meters of target
