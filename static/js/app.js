(() => {
  ...
  const mapLayers = {
    trips: { order: 1, color: '#BB86FC', opacity: 0.4, visible: true, layer: null },
    historicalTrips: { order: 2, color: '#03DAC6', opacity: 0.4, visible: false, layer: null },
    matchedTrips: { order: 3, color: '#CF6679', opacity: 0.4, visible: false, layer: null },
  ...
  let map, layerGroup, liveTracker, mapInitialized = false;
  ...
  class LoadingManager {
    ...
    updateOverallProgress() {
      this.totalProgress = Object.values(this.operations).reduce((acc, op) => acc + op.progress / 100, 0);
  ...
  function initializeDatePickers() {
    ...
    const config = { dateFormat: 'Y-m-d', maxDate: tomorrow, enableTime: false, static: true };
  ...
  function handleDatePresetClick() {
    ...
    if (range === 'all-time') {
      ...
      fetch('/api/first_trip_date')
        .then(r => r.json())
        .then(d => updateDatePickersAndStore(new Date(d.first_trip_date), endDate))
  ...
  function visualizeStreetCoverage(coverageData) {
    ...
    mapLayers.streetCoverage.layer = L.geoJSON(coverageData.streets_data, {
      ...
      onEachFeature: (feature, layer) => {
        layer.on('click', () => fetchSegmentDetails(feature.properties.segment_id));
      }
    });
  ...
  function updateCoverageStats(coverageData) {
    ...
    const percent = coverageData.coverage_percentage,
      ...
      drivenLengthMiles = coverageData.streets_data.metadata.driven_length_miles;
