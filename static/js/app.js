...
(() => {
    ...
    async function fetchTrips() {
      ...
      loadingManager.addSubOperation('fetch', 50);
      loadingManager.addSubOperation('display', 30);
      loadingManager.addSubOperation('matched', 20);
      ...
      try {
        ...
        loadingManager.updateSubOperation('fetch', 25);
        ...
        loadingManager.updateSubOperation('display', 15);
        ...
        loadingManager.updateSubOperation('display', 30);
  
        // Matched trips
        loadingManager.updateSubOperation('matched', 10);
        try {
          await fetchMatchedTrips();
        } catch (error) {
          console.error('Error fetching matched trips:', error);
        } finally {
          loadingManager.updateSubOperation('matched', 20);
        }
      } catch (error) {
      ...
      for (const [layerName, layerInfo] of visibleLayers) {
        ...
        await new Promise((resolve) => {
          if (layerName === 'streetCoverage' || layerName === 'customPlaces') {
            ...
            layerInfo.layer
              .setStyle({ color: layerInfo.color, opacity: layerInfo.opacity })
              .addTo(layerGroup);
            resolve();
          } else {
            resolve();
      ...
      for (const [layerName, layerInfo] of Object.entries(mapLayers)) {
        ...
        layerControl.innerHTML = `
          <label class="custom-checkbox">
            <input type="checkbox" id="${layerName}-toggle" ${
          layerInfo.visible ? 'checked' : ''
        }>
