(() => {
    ...
    function initializeEventListeners() {
      // Apply Filters button: update date values and trigger fetches.
    ...
    // Updated: When a date preset button is clicked, update the date inputs and store the values—but do not trigger fetches.
    function handleDatePresetClick() {
      ...
      updateDatePickersAndStore(startDate, endDate);
      // Do not automatically fetch trips or metrics—wait for the user to click "Apply Filters".
    }
  
    // Updated: This function only updates the date inputs and saves to localStorage.
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // If there's a map (and not on visits page) do map initialization
      if (document.getElementById('map') && !document.getElementById('visits-page')) {
        ...
        // On page load, fetch trips and metrics (to have an initial view)
        fetchTrips().then(() => {
          fetchMetrics();
        });
      } else {
