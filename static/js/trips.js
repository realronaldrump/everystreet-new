        ...
        columns: [
            ...
            { 
             ...
             { 
                data: "duration", 
                name: "duration",
                render: function(data) { return formatDuration(data); }
            },
            { 
                data: "distance", 
                name: "distance",
                render: function(data) { return data ? `${parseFloat(data).toFixed(1)} mi` : '--'; }
            },
            ...
            { 
                data: "maxSpeed", 
                name: "maxSpeed",
                render: function(data) { return data ? `${Math.round(data)} mph` : '--'; }
            },
             { 
            ...
            { 
                data: "fuelConsumed", 
                name: "fuelConsumed",
                 render: function(data) { return data ? `${parseFloat(data).toFixed(2)} gal` : '--'; }
            },
            { 
                data: "estimated_cost", 
                name: "estimated_cost",
...
function setupRowActions() {
    $('.delete-trip-btn').off('click').on('click', async function() {
    ...
    $('.trip-checkbox').off('change').on('change', function() {
        updateBulkDeleteButton();
    });
}
...
function setupBulkActions() {
    $('#bulk-delete-trips-btn').on('click', async function() {
        const selectedIds = $('.trip-checkbox:checked').map(function() {
            return $(this).val();
        }).get();
    ...
    $('#refresh-geocoding-btn').on('click', async function() {
        ...
        try {
            if (window.notificationManager) window.notificationManager.show("Starting geocoding refresh...", "info");
            ...
            if (response.ok) {
                 if (window.notificationManager) window.notificationManager.show("Geocoding task started.", "success");
            } else {
                 throw new Error(result.detail || "Failed to start geocoding");
            }
        } catch(e) {
             if (window.notificationManager) window.notificationManager.show(e.message, "danger");
        }
    });
}
...
async function deleteTrip(id) {
    try {
        ...
        if (window.notificationManager) window.notificationManager.show("Trip deleted successfully", "success");
        tripsTable.ajax.reload(null, false); // Reload but keep page
    } catch (e) {
        console.error("Delete failed", e);
        if (window.notificationManager) window.notificationManager.show("Failed to delete trip", "danger");
    }
}
...
async function bulkDeleteTrips(ids) {
    try {
        ...
        if (window.notificationManager) window.notificationManager.show(result.message || "Trips deleted", "success");
        tripsTable.ajax.reload(null, false);
        $('#select-all-trips').prop('checked', false);
    } catch (e) {
         console.error("Bulk delete failed", e);
        if (window.notificationManager) window.notificationManager.show("Failed to delete trips", "danger");
    }
}
