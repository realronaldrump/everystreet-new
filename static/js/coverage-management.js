...
(() => {
  class CoverageManager {
    ...
    initializeKeyboardShortcuts() {
      this.keyboardShortcuts.set('ctrl+n', () => {
        const modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('addAreaModal'));
        modal.show();
      });
    ...
    processRenderQueue() {
      ...
      while (this.renderQueue.length > 0 && performance.now() - startTime < this.frameTime * 0.8) {
    ...
    static setupConnectionMonitoring() {
      ...
      const handleConnectionChange = () => {
        ...
        if (!isOnline) {
          ...
          if (window.coverageManager) { // Assuming coverageManager is global
    ...
    initializeQuickActions() {
      // Quick refresh all
      document.getElementById('quick-refresh-all')?.addEventListener('click', async () => {
        const areas = await this.getAllCoverageAreas();
        if (areas.length === 0) {
          this.notificationManager.show("No coverage areas to refresh", "info");
          return;
        }
    ...
    async batchUpdateCoverageAreas(areas) {
      ...
      for (const area of areas) {
        try {
          ...
          console.error(`Failed to update ${area.location?.display_name}:`, error);
          failed++;
        }
    ...
    async exportAllCoverageData() {
      ...
      this.notificationManager.show("Coverage data exported successfully", "success");
    }
    ...
    handleTableAction(button) {
      ...
      switch (action) {
        ...
        case "delete":
          if (locationData) {
            this.deleteArea(locationData)
              .finally(resetButton);
          }
          break;
        case "cancel":
          if (locationData) {
            this.cancelProcessing(locationData)
              .finally(resetButton);
          }
          break;
        default:
    ...
    showInterruptedTaskNotification(progressData) {
      ...
      notification.className = "alert alert-info alert-dismissible fade show mt-3 fade-in-up";
    ...
    async updateCoverageForArea(locationId, mode = "full", showNotification = true) {
      ...
      if (
        this.currentProcessingLocation?.display_name === locationData.display_name
      ) {
        this.notificationManager.show(
      ...
      try {
        ...
        const isUpdatingDisplayedLocation = this.selectedLocation?._id === locationId;
    ...
    async deleteArea(location) {
      ...
      const confirmed = await this.confirmationDialog.show({
        ...
        details: "This will permanently delete all associated street data, statistics, and history. This action cannot be undone.",
    ...
    updateModalContent(data) {
      ...
      if ([STATUS.COMPLETE, STATUS.COMPLETED, STATUS.ERROR, STATUS.CANCELED].includes(stage)) {
        if (this.progressTimer) {
          clearInterval(this.progressTimer);
          this.progressTimer = null;
          this.updateTimingInfo();
        ...
        if (!this.progressTimer) {
          this.processingStartTime = Date.now() - (this.lastProgressUpdate?.elapsedMs || 0);
          this.progressTimer = setInterval(() => {
            this.updateTimingInfo();
            this.updateActivityIndicator();
          }, 1000);
    ...
    static updateStepIndicators(stage, progress) {
      ...
      if (stage === STATUS.ERROR) {
        ...
        if (progress > 75 && steps.calculating) { markError("calculating"); errorStepFound = true; }
        else if (progress > 50 && steps.indexing) { markError("indexing"); errorStepFound = true; }
        else if (progress > 10 && steps.preprocessing) { markError("preprocessing"); errorStepFound = true; }
        else if (steps.initializing) { markError("initializing"); errorStepFound = true; }
        
        // Mark previous steps as complete if error occurred later
        if (errorStepFound) {
            if (steps.calculating.classList.contains('error') && steps.indexing) markComplete('indexing');
            if ((steps.calculating.classList.contains('error') || steps.indexing.classList.contains('error')) && steps.preprocessing) markComplete('preprocessing');
            if ((steps.calculating.classList.contains('error') || steps.indexing.classList.contains('error') || steps.preprocessing.classList.contains('error')) && steps.initializing) markComplete('initializing');
        }
        return;
      }
      ...
      if (stage === STATUS.CANCELED) {
        ...
        if (steps.calculating && steps.calculating.classList.contains('active')) markError('calculating');
        else if (steps.indexing && steps.indexing.classList.contains('active')) markError('indexing');
        else if (steps.preprocessing && steps.preprocessing.classList.contains('active')) markError('preprocessing');
      ...
      switch (stage) {
        ...
        default: // Fallback for unknown stages or if progress is the only guide
          if (progress >= 100) {
            markComplete("initializing"); markComplete("preprocessing"); markComplete("indexing"); markComplete("calculating"); markComplete("complete");
          } else if (progress > 75) {
            markComplete("initializing"); markComplete("preprocessing"); markComplete("indexing"); markActive("calculating");
          } else if (progress > 50) {
            markComplete("initializing"); markComplete("preprocessing"); markActive("indexing");
          } else if (progress > 10 || stage?.toLowerCase().includes("preprocessing")) {
            markComplete("initializing"); markActive("preprocessing");
          } else {
            markActive("initializing");
          }
          break;
    ...
    updateTimingInfo() {
      ...
      const estimatedTimeEl = document.querySelector( // For future use
    ...
    updateActivityIndicator(isActive = null) {
      ...
      if (isActive !== null) {
        ...
        currentlyActive = this.lastActivityTime && (now - this.lastActivityTime < 10000); // 10s threshold
      }
    ...
    formatMetricStats(stage, metrics) {
      ...
      const addStat = (label, value, unit = "", icon = null, colorClass = "text-primary") => {
      ...
      if (metrics.total_length_m !== undefined) {
        addStat("Total Length", CoverageManager.distanceInUserUnits(metrics.total_length_m), "", "fas fa-ruler-horizontal");
      }
       if (metrics.driveable_length_m !== undefined) {
        addStat("Driveable Length", CoverageManager.distanceInUserUnits(metrics.driveable_length_m), "", "fas fa-car");
      }

      // Stage-specific stats
      if ([STATUS.INDEXING, STATUS.PREPROCESSING, STATUS.LOADING_STREETS, STATUS.POST_PREPROCESSING].includes(stage)) {
        if (metrics.initial_covered_segments !== undefined) {
          addStat("Initial Driven", metrics.initial_covered_segments, " segs", "fas fa-flag-checkered", "text-success");
        }
      } else if ([STATUS.PROCESSING_TRIPS, STATUS.CALCULATING, STATUS.COUNTING_TRIPS].includes(stage)) {
        ...
        addStat("Trips Processed", `${processed.toLocaleString()}/${total.toLocaleString()} (${tripsProgress}%)`, "", "fas fa-route", "text-info");
        if (metrics.newly_covered_segments !== undefined) {
          addStat("New Segments Found", metrics.newly_covered_segments, "", "fas fa-plus-circle", "text-success");
        }
        if (metrics.coverage_percentage !== undefined) {
          addStat("Current Coverage", metrics.coverage_percentage.toFixed(1), "%", "fas fa-tachometer-alt", "text-success");
        }
        if (metrics.covered_length_m !== undefined) {
          addStat("Distance Covered", CoverageManager.distanceInUserUnits(metrics.covered_length_m), "", "fas fa-road", "text-success");
        }
      } else if ([STATUS.FINALIZING, STATUS.GENERATING_GEOJSON, STATUS.COMPLETE_STATS, STATUS.COMPLETE, STATUS.COMPLETED].includes(stage)) {
        const finalCovered = metrics.total_covered_segments || metrics.covered_segments;
        if (finalCovered !== undefined) {
          addStat("Segments Covered", finalCovered, "", "fas fa-check-circle", "text-success");
        }
        if (metrics.coverage_percentage !== undefined) {
          addStat("Final Coverage", metrics.coverage_percentage.toFixed(1), "%", "fas fa-check-double", "text-success");
        }
        if (metrics.covered_length_m !== undefined) {
          addStat("Distance Covered", CoverageManager.distanceInUserUnits(metrics.covered_length_m), "", "fas fa-road", "text-success");
        }
    ...
    async displayCoverageDashboard(locationId) {
      ...
      const streetTypeCoverageElement = document.getElementById("street-type-coverage");
      const mapContainer = document.getElementById("coverage-map");

      if (!dashboardElement || !locationNameElement || !mapContainer) {
        console.error("Essential dashboard elements not found.");
        this.notificationManager.show("UI Error: Dashboard components missing.", "danger");
        return;
      }
      ...
      if (streetTypeChartElement) streetTypeChartElement.innerHTML = CoverageManager.createLoadingSkeleton(180);
      if (streetTypeCoverageElement) streetTypeCoverageElement.innerHTML = CoverageManager.createLoadingSkeleton(100, 3);
      mapContainer.innerHTML = CoverageManager.createLoadingIndicator("Loading map data...");
      ...
      try {
        ...
        if (cachedData) {
            ...
            if (!metaResponse.ok) {
                ...
                throw new Error(`Failed to load metadata: ${errorData.detail || metaResponse.statusText}`);
            }
            const apiResponse = await metaResponse.json();
            if (!apiResponse.success || !apiResponse.coverage || !apiResponse.coverage.location) {
                throw new Error(apiResponse.error || "Incomplete metadata received.");
            }
            coverageData = apiResponse.coverage;
            
            const streetsResp = await fetch(`/api/coverage_areas/${locationId}/streets?cache_bust=${new Date().getTime()}`);
            if (!streetsResp.ok) {
                const errData = await streetsResp.json().catch(() => ({}));
                throw new Error(`Failed to load street geometry: ${errData.detail || streetsResp.statusText}`);
            }
        ...
        locationNameElement.textContent = coverageData.location.display_name || "Unnamed Area";
        ...
        if (coverageData.streets_geojson) {
            ...
            mapContainer.innerHTML = CoverageManager.createAlertMessage("Map Data Error", "Could not load street geometry.", "danger", locationId);
        }
        ...
        this.notificationManager.show(`Error loading dashboard: ${error.message}`, "danger");
        mapContainer.innerHTML = CoverageManager.createAlertMessage("Dashboard Load Error", error.message, "danger", locationId);
      } finally {
        this.initTooltips();
      }
    }
    ...
    updateDashboardStats(coverage) {
      if (!coverage) return;
      const statsContainer = document.querySelector(".dashboard-stats-card .stats-container");
      ...
      if (coverage.status === STATUS.ERROR || coverage.status === STATUS.CANCELED) barColor = "bg-danger";
      else if (coverage.status !== STATUS.COMPLETED && coverage.status !== STATUS.COMPLETE) barColor = "bg-warning";
    ...
    updateStreetTypeCoverage(streetTypes) {
      ...
      if (!streetTypes || !streetTypes.length) {
        streetTypeCoverageEl.innerHTML = CoverageManager.createAlertMessage("No Data", "No street type data available.", "secondary");
        return;
      }
      ...
      topTypes.forEach((type) => {
        ...
        const coveredDist = CoverageManager.distanceInUserUnits(parseFloat(type.covered_length_m || 0));
        const totalDist = CoverageManager.distanceInUserUnits(parseFloat((type.driveable_length_m !== undefined ? type.driveable_length_m : type.total_length_m) || 0));
    ...
    clearDashboardUI() {
      document.getElementById("dashboard-location-name").textContent = "Select a location";
      const statsContainer = document.querySelector(".dashboard-stats-card .stats-container");
      ...
      if (this.coverageMap) {
        try { this.coverageMap.remove(); } catch(e) { /* ignore */ }
        this.coverageMap = null;
      }
      ...
      if (this.coverageSummaryControl && this.coverageMap) {
        try { this.coverageMap.removeControl(this.coverageSummaryControl); } catch (e) { /* ignore */ }
        this.coverageSummaryControl = null;
      }
    ...
    static createAlertMessage(title, message, type = "info", locationId = null) {
      const iconClass = {
        danger: "fa-exclamation-circle",
        warning: "fa-exclamation-triangle",
        info: "fa-info-circle",
    ...
    initializeCoverageMap(coverage) {
      ...
      if (this.coverageMap && typeof this.coverageMap.remove === "function") {
        try { this.coverageMap.remove(); } catch (e) { console.warn("Error removing previous map:", e); }
        this.coverageMap = null;
      }
      ...
      try {
        ...
        this.coverageMap.addControl(new mapboxgl.NavigationControl(), 'top-right');
        this.coverageMap.addControl(new mapboxgl.ScaleControl());
        this.coverageMap.addControl(new mapboxgl.FullscreenControl());
        this.coverageMap.addControl(new mapboxgl.AttributionControl({ compact: true }), "bottom-right");

        this.coverageMap.on("load", () => {
          if (coverage.streets_geojson) {
            this.addStreetsToMap(coverage.streets_geojson);
          } else {
            this.notificationManager.show("No street data found for this area.", "warning");
            this.mapBounds = null;
          }
          ...
          if (this.showTripsActive) { // If trip overlay was active, re-setup and load
              this.setupTripLayers();
              this.loadTripsForView();
          }
        });
        ...
        this.coverageMap.on("error", (e) => {
          ...
          mapContainer.innerHTML = CoverageManager.createAlertMessage("Map Load Error", e.error?.message || "Could not initialize map.", "danger");
        });
    ...
    addStreetsToMap(geojson) {
      ...
      layersToRemove.forEach(layerId => {
        if (this.coverageMap.getLayer(layerId)) this.coverageMap.removeLayer(layerId);
      });
      if (this.coverageMap.getSource("streets")) this.coverageMap.removeSource("streets");
      ...
      try {
        ...
        const getLineColor = [
          "case",
          ["boolean", ["feature-state", "hover"], false], "#ffff00", // Yellow hover
          ["boolean", ["get", "undriveable"], false], "#607d8b", // Grey undriveable
          ["boolean", ["get", "driven"], false], "#4caf50", // Green driven
          "#ff5252" // Red not driven
        ];
        const getLineWidth = ["interpolate", ["linear"], ["zoom"], 8, 1.5, 14, 4, 18, 7];
        const getLineOpacity = ["case", ["boolean", ["feature-state", "hover"], false], 1.0, ["boolean", ["get", "undriveable"], false], 0.6, 0.85];
        const getLineDash = ["case", ["boolean", ["get", "undriveable"], false], ["literal", [2, 2]], ["literal", [1, 0]]];

        this.coverageMap.addLayer({
          id: "streets-layer", type: "line", source: "streets",
          layout: { "line-cap": "round", "line-join": "round" },
          paint: { "line-color": getLineColor, "line-width": getLineWidth, "line-opacity": getLineOpacity, "line-dasharray": getLineDash }
        });

        const bounds = new mapboxgl.LngLatBounds();
        geojson.features.forEach(f => {
          if (f.geometry?.coordinates) {
            if (f.geometry.type === "LineString") f.geometry.coordinates.forEach(coord => bounds.extend(coord));
            else if (f.geometry.type === "MultiLineString") f.geometry.coordinates.forEach(line => line.forEach(coord => bounds.extend(coord)));
          }
        });
          ...
          if (e.features?.length > 0) {
            ...
            if (currentHoverId !== hoveredSegmentId) {
              if (hoveredSegmentId !== null && this.coverageMap.getSource("streets")) {
                this.coverageMap.setFeatureState({ source: "streets", id: hoveredSegmentId }, { hover: false });
              }
              if (this.coverageMap.getSource("streets")) {
                this.coverageMap.setFeatureState({ source: "streets", id: currentHoverId }, { hover: true });
              }
        ...
        this.coverageMap.on("click", "streets-layer", (e) => {
          ...
          if (e.features?.length > 0) {
            ...
            const popup = new mapboxgl.Popup({ closeButton: true, closeOnClick: true, maxWidth: "350px", className: "coverage-popup" })
              .setLngLat(e.lngLat).setHTML(popupContent).addTo(this.coverageMap);
        ...
        this.notificationManager.show(`Failed to display streets: ${error.message}`, "danger");
      }
    }
    ...
    createStreetPopupContentHTML(props) {
      ...
      const lengthFormatted = CoverageManager.distanceInUserUnits(segmentLength);
    ...
    async _handleMarkSegmentAction(action, segmentId) {
      ...
      switch (action) {
        case "driven": endpoint = "/api/street_segments/mark_driven"; break;
        case "undriven": endpoint = "/api/street_segments/mark_undriven"; break;
        case "undriveable": endpoint = "/api/street_segments/mark_undriveable"; break;
        case "driveable": endpoint = "/api/street_segments/mark_driveable"; break;
        default: this.notificationManager.show(`Unknown action: ${action}`, "warning"); return;
      }

      try {
        await this._makeSegmentApiRequest(endpoint, payload);
        this.notificationManager.show(`Segment marked as ${action}. Refreshing...`, "success", 2000);

        // Optimistic UI update
        if (this.streetsGeoJson?.features && this.coverageMap?.getSource("streets")) {
          ...
          if (featureIndex !== -1) {
            ...
            switch (action) {
              case "driven": feature.properties.driven = true; feature.properties.undriveable = false; break;
              case "undriven": feature.properties.driven = false; break;
              case "undriveable": feature.properties.undriveable = true; feature.properties.driven = false; break;
              case "driveable": feature.properties.undriveable = false; break;
            }
        ...
        this.notificationManager.show(`Failed to mark segment: ${error.message}`, "danger");
      }
    }
    
    async refreshDashboardData(locationId) {
        try {
            const refreshResp = await fetch(`/api/coverage_areas/${locationId}/refresh_stats`, { method: "POST" });
            ...
            if (refreshResp.ok && refreshData.coverage) {
                ...
                this.updateStreetTypeCoverage(refreshData.coverage.street_types || []);
                ...
                this.notificationManager.show(`Failed to refresh stats: ${refreshData.detail || "Unknown error"}`, "warning");
            }
        } catch (e) {
    ...
    async _makeSegmentApiRequest(endpoint, payload) {
      try {
        ...
        const data = await response.json();
        if (!response.ok) throw new Error(data.detail || `API request failed (HTTP ${response.status})`);
        return data;
      } catch (error) {
    ...
    setMapFilter(filterType, updateButtons = true) {
      if (!this.coverageMap || !this.coverageMap.getLayer("streets-layer")) return;
      this.currentFilter = filterType;
      let filter = null;

      if (filterType === "driven") filter = ["all", ["==", ["get", "driven"], true], ["!=", ["get", "undriveable"], true]];
      else if (filterType === "undriven") filter = ["all", ["==", ["get", "driven"], false], ["!=", ["get", "undriveable"], true]];
    ...
    updateFilterButtonStates() {
      const filterButtons = document.querySelectorAll('.map-controls button[data-filter]');
      filterButtons.forEach(btn => {
        btn.classList.remove("active", "btn-primary", "btn-success", "btn-danger");
        btn.classList.add(btn.dataset.filter === this.currentFilter ? 
            (this.currentFilter === "driven" ? "btn-success" : this.currentFilter === "undriven" ? "btn-danger" : "btn-primary") :
            (btn.dataset.filter === "driven" ? "btn-outline-success" : btn.dataset.filter === "undriven" ? "btn-outline-danger" : "btn-outline-primary")
        );
    ...
    static getStageIcon(stage) {
      const icons = {
        [STATUS.INITIALIZING]: '<i class="fas fa-cog fa-spin"></i>', [STATUS.PREPROCESSING]: '<i class="fas fa-map-marked-alt"></i>',
        [STATUS.LOADING_STREETS]: '<i class="fas fa-map"></i>', [STATUS.INDEXING]: '<i class="fas fa-project-diagram"></i>',
        [STATUS.COUNTING_TRIPS]: '<i class="fas fa-calculator"></i>', [STATUS.PROCESSING_TRIPS]: '<i class="fas fa-route fa-spin"></i>',
        [STATUS.CALCULATING]: '<i class="fas fa-cogs fa-spin"></i>', [STATUS.FINALIZING]: '<i class="fas fa-chart-line"></i>',
        [STATUS.GENERATING_GEOJSON]: '<i class="fas fa-file-code fa-spin"></i>', [STATUS.COMPLETE_STATS]: '<i class="fas fa-check"></i>',
        [STATUS.COMPLETE]: '<i class="fas fa-check-circle"></i>', [STATUS.COMPLETED]: '<i class="fas fa-check-circle"></i>',
        [STATUS.ERROR]: '<i class="fas fa-exclamation-circle"></i>', [STATUS.WARNING]: '<i class="fas fa-exclamation-triangle"></i>',
        [STATUS.CANCELED]: '<i class="fas fa-ban"></i>', [STATUS.POLLING_CHECK]: '<i class="fas fa-sync-alt fa-spin"></i>',
        [STATUS.UNKNOWN]: '<i class="fas fa-question-circle"></i>', [STATUS.POST_PREPROCESSING]: '<i class="fas fa-cog fa-spin"></i>',
      };
    ...
    static getStageTextClass(stage) {
      const classes = {
        [STATUS.COMPLETE]: "text-success", [STATUS.COMPLETED]: "text-success", [STATUS.ERROR]: "text-danger",
        [STATUS.WARNING]: "text-warning", [STATUS.CANCELED]: "text-warning", [STATUS.POST_PREPROCESSING]: "text-info",
      };
    ...
    static formatStageName(stage) {
      const stageNames = {
        [STATUS.INITIALIZING]: "Initializing", [STATUS.PREPROCESSING]: "Fetching Streets", [STATUS.LOADING_STREETS]: "Loading Streets",
        [STATUS.INDEXING]: "Building Index", [STATUS.COUNTING_TRIPS]: "Analyzing Trips", [STATUS.PROCESSING_TRIPS]: "Processing Trips",
        [STATUS.CALCULATING]: "Calculating Coverage", [STATUS.FINALIZING]: "Calculating Stats", [STATUS.GENERATING_GEOJSON]: "Generating Map",
        [STATUS.COMPLETE_STATS]: "Finalizing", [STATUS.COMPLETE]: "Complete", [STATUS.COMPLETED]: "Complete",
        [STATUS.ERROR]: "Error", [STATUS.WARNING]: "Warning", [STATUS.CANCELED]: "Canceled",
        [STATUS.POLLING_CHECK]: "Checking Status", [STATUS.UNKNOWN]: "Unknown", [STATUS.POST_PREPROCESSING]: "Post-processing",
      };
    ...
    setupTripLayers() {
      ...
      if (!this.coverageMap.getSource("trips-source")) {
        this.coverageMap.addSource("trips-source", { type: "geojson", data: { type: "FeatureCollection", features: [] } });
      }
      if (!this.coverageMap.getLayer("trips-layer")) {
        this.coverageMap.addLayer({
          id: "trips-layer", type: "line", source: "trips-source",
          layout: { "line-join": "round", "line-cap": "round" },
          paint: { "line-color": "#3388ff", "line-width": 2.5, "line-opacity": 0.75, "line-blur": 0.5 }
        }, "streets-layer"); // Add below streets
      }
    }
    ...
    clearTripOverlay() {
      ...
      try {
        this.coverageMap.getSource("trips-source").setData({ type: "FeatureCollection", features: [] });
      } catch (error) { console.warn("Error clearing trip overlay:", error); }
    }

    async loadTripsForView() {
      if (!this.coverageMap || !this.showTripsActive || !this.coverageMap.isStyleLoaded()) return;
      this.setupTripLayers();
      ...
      if (zoom < 12) { // Only load trips if zoomed in sufficiently
        this.notificationManager.show("Zoom in further to view trip overlays.", "info", 2000);
        this.clearTripOverlay(); return;
      }

      const params = new URLSearchParams({ min_lat: sw.lat.toFixed(6), min_lon: sw.lng.toFixed(6), max_lat: ne.lat.toFixed(6), max_lon: ne.lng.toFixed(6) });
      try {
        ...
        const data = await response.json();
        if (!data || !Array.isArray(data.trips)) throw new Error("Invalid trip data received.");

        const tripFeatures = data.trips.map((coords, index) => {
          if (!Array.isArray(coords) || coords.length < 2 || !Array.isArray(coords[0]) || coords[0].length < 2) return null;
          return { type: "Feature", properties: { tripId: `trip-${index}` }, geometry: { type: "LineString", coordinates: coords } };
        }).filter(feature => feature !== null);

        tripsSource.setData({ type: "FeatureCollection", features: tripFeatures });
      } catch (error) {
        this.notificationManager.show(`Failed to load trip overlay: ${error.message}`, "danger");
        this.clearTripOverlay();
      }
    }
    ...
    createStreetTypeChart(streetTypes) {
      ...
      if (!streetTypes || !streetTypes.length) {
        chartContainer.innerHTML = CoverageManager.createAlertMessage("No Data", "No street type data available for chart.", "secondary");
        return;
      }

      const sortedTypes = [...streetTypes].sort((a, b) => (b.total_length_m || 0) - (a.total_length_m || 0)).slice(0, 10); // Top 10 types
      const labels = sortedTypes.map(t => CoverageManager.formatStreetType(t.type));
      ...
      chartContainer.innerHTML = '<canvas id="streetTypeChartCanvas" style="min-height: 180px;"></canvas>';
      const ctx = document.getElementById("streetTypeChartCanvas").getContext("2d");

      this.streetTypeChartInstance = new Chart(ctx, {
        type: "bar",
        data: { labels, datasets: [
            { label: "Covered (mi)", data: covered, backgroundColor: "#4caf50", order: 1 },
            { label: "Driveable (mi)", data: driveable, backgroundColor: "#607d8b", order: 1 },
            { label: "% Covered", data: coveragePct, type: "line", yAxisID: "y1", borderColor: "#ffb300", tension: 0.2, order: 0 }
        ]},
        options: {
          responsive: true, maintainAspectRatio: false,
          plugins: { legend: { position: "bottom", labels: { color: "#fff", boxWidth: 15, padding: 15 } },
            tooltip: { mode: 'index', intersect: false, callbacks: {
              label: ctx => `${ctx.dataset.label}: ${ctx.dataset.label === "% Covered" ? ctx.parsed.y.toFixed(1) + "%" : ctx.parsed.y.toFixed(2) + " mi"}`
            }}
          },
          scales: {
            x: { ticks: { color: "#ccc", font: {size: 10} }, grid: { color: "rgba(255,255,255,0.05)" } },
            y: { beginAtZero: true, title: { display: true, text: "Distance (mi)", color: "#ccc" }, ticks: { color: "#ccc" }, grid: { color: "rgba(255,255,255,0.1)" } },
            y1: { beginAtZero: true, position: "right", title: { display: true, text: "% Covered", color: "#ffb300" }, ticks: { color: "#ffb300", callback: v => v + "%" }, grid: { drawOnChartArea: false }, min: 0, max: 100 }
          }
        }
      });
    }
    ...
    addCoverageSummary(coverage) {
      ...
      this.coverageSummaryControl = { onAdd: () => controlDiv, onRemove: () => controlDiv.remove(), getDefaultPosition: () => "top-left" };
      try { this.coverageMap.addControl(this.coverageSummaryControl, "top-left"); } 
      catch (e) { console.error("Error adding coverage summary control:", e); }
    }

    exportCoverageMap() {
      const mapContainer = document.getElementById("coverage-map");
      if (!this.coverageMap || !mapContainer) {
        this.notificationManager.show("Map not ready for export.", "warning"); return;
      }
      this.notificationManager.show("Preparing map export...", "info");

      const doExport = () => {
        setTimeout(() => { // Allow map to render fully
          html2canvas(mapContainer, { useCORS: true, backgroundColor: "#1e1e1e", logging: false, allowTaint: true, width: mapContainer.offsetWidth, height: mapContainer.offsetHeight })
            .then(canvas => {
              canvas.toBlob(blob => {
                if (!blob) { this.notificationManager.show("Failed to create image blob.", "danger"); return; }
                const url = URL.createObjectURL(blob);
                const a = document.createElement("a");
                a.href = url;
                const locationName = this.selectedLocation?.location?.display_name || "coverage_map";
                const dateStr = new Date().toISOString().split("T")[0];
                a.download = `${locationName.replace(/[^a-z0-9]/gi, "_").toLowerCase()}_${dateStr}.png`;
                document.body.appendChild(a); a.click();
                setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(url); this.notificationManager.show("Map exported.", "success"); }, 100);
              }, "image/png");
            }).catch(error => {
              console.error("html2canvas export error:", error);
              this.notificationManager.show(`Map export failed: ${error.message}`, "danger");
            });
        }, 500);
      };
      ...
      if (typeof html2canvas === "undefined") {
        ...
        script.integrity = "sha512-BNaRQnYJYiPSqHHDb58B0yaPfCu+Wgds8Gp/gU33kqBtgP4CX_NTClWb89zZNRaYQzcbwSSsqrPDRRMKASBbA==";
        script.crossOrigin = "anonymous";
        script.onload = doExport;
        script.onerror = () => this.notificationManager.show("Failed to load export library.", "danger");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    if (typeof mapboxgl === "undefined") {
      const msg = "Error: Mapbox GL JS library failed to load. Map functionality will be unavailable.";
      const errContainer = document.getElementById("alerts-container") || document.body;
      const errDiv = document.createElement("div");
      errDiv.className = "alert alert-danger m-3"; errDiv.textContent = msg;
    ...
    if (typeof Chart === "undefined") {
      ...
      const chartContainer = document.getElementById("street-type-chart");
      if (chartContainer) chartContainer.innerHTML = '<div class="alert alert-warning small p-2">Chart library not loaded.</div>';
    }
