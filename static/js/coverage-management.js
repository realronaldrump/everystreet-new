...
class CoverageManager {
  constructor() {
    ...
    this.map.onSegmentSelection = (segmentId) => this.toggleSegmentSelection(segmentId);
    this.map.handleSegmentAction = (action, segmentId) => this.handleMarkSegmentAction(action, segmentId);
  ...
  setupEventListeners() {
    // Quick actions
    document.getElementById('quick-refresh-all')?.addEventListener('click', () => this.batchRefreshAll());
    document.getElementById('quick-export-data')?.addEventListener('click', () => this.exportAllCoverageData());
    document.getElementById('refresh-table-btn')?.addEventListener('click', () => this.loadCoverageAreas(true));
    document.getElementById('close-dashboard-btn')?.addEventListener('click', () => this.closeCoverageDashboard());
    document.getElementById('find-efficient-street-btn')?.addEventListener('click', () => this.findMostEfficientStreets());
    ...
    document.querySelectorAll('input[name="area-definition-type"]').forEach(radio => {
      radio.addEventListener('change', (e) => this.handleAreaDefinitionTypeChange(e.target.value));
    });
    ...
    document.getElementById('undriven-streets-sort')?.addEventListener('change', (e) => {
  ...
  async loadCoverageAreas(showLoading = true, silent = false) {
    ...
    try {
      ...
      if (!silent) {
        window.notificationManager.show(`Failed to load coverage areas: ${error.message}`, 'danger');
      }
  ...
  createAreaTableRow(area, index) {
    ...
    const isProcessing = ['processing_trips', 'preprocessing', 'calculating', 'indexing', 'finalizing'].includes(status);
    const hasError = status === 'error';
    const isCanceled = status === 'canceled';

    row.className = isProcessing ? 'processing-row table-info' 
      : hasError ? 'table-danger' 
      : isCanceled ? 'table-warning' : '';
    ...
    const totalLength = this.ui.constructor.distanceInUserUnits(area.total_length);
    const drivenLength = this.ui.constructor.distanceInUserUnits(area.driven_length);
    const coveragePercentage = area.coverage_percentage?.toFixed(1) || '0.0';

    const progressBarColor = hasError || isCanceled ? 'bg-secondary'
      : area.coverage_percentage < 25 ? 'bg-danger'
      : area.coverage_percentage < 75 ? 'bg-warning' : 'bg-success';
  ...
  handleTableAction(button) {
    ...
    if (!locationId && !locationStr) {
      window.notificationManager.show('Action failed: Missing location identifier.', 'danger');
      return;
    }
    ...
    if (locationStr) {
      try {
        ...
        window.notificationManager.show('Action failed: Invalid location data.', 'danger');
        return;
      }
    }
    ...
    const actions = {
      'update-full': () => locationId ? this.updateCoverageForArea(locationId, 'full') : Promise.resolve(),
      'update-incremental': () => locationId ? this.updateCoverageForArea(locationId, 'incremental') : Promise.resolve(),
      'delete': () => locationData ? this.deleteArea(locationData) : Promise.resolve(),
      'cancel': () => locationData ? this.cancelProcessing(locationData) : Promise.resolve(),
      'reprocess': () => locationId ? this.reprocessStreetsForArea(locationId) : Promise.resolve()
    };
  ...
  async updateCoverageForArea(locationId, mode = 'full', showNotification = true) {
    try {
      ...
      if (taskData?.task_id) {
        ...
        if (showNotification) {
          window.notificationManager.show(`Coverage update completed for ${locationData.display_name}`, 'success');
        }
      ...
      if (showNotification) {
        window.notificationManager.show(`Coverage update failed: ${error.message}`, 'danger');
      }
      throw error;
    }
  }
  ...
  async deleteArea(location) {
    const confirmed = await this.modal.showConfirmDialog({
      ...
      details: 'This will permanently delete all associated data. This action cannot be undone.',
    ...
    try {
      ...
      if (this.state.selectedLocation?.location?.display_name === location.display_name) {
        this.closeCoverageDashboard();
      }
      
      window.notificationManager.show(`Coverage area '${location.display_name}' deleted.`, 'success');
      this.updateTotalAreasCount();
    } catch (error) {
      console.error('Error deleting coverage area:', error);
      window.notificationManager.show(`Error deleting coverage area: ${error.message}`, 'danger');
    }
  }
  ...
  async cancelProcessing(location = null) {
    ...
    if (!locationToCancel?.display_name) {
      window.notificationManager.show('No active processing to cancel.', 'warning');
      return;
    }
    ...
    try {
      ...
      window.notificationManager.show(`Processing cancelled for ${locationToCancel.display_name}`, 'success');
      
      if (this.state.currentProcessingLocation?.display_name === locationToCancel.display_name) {
      ...
      window.notificationManager.show(`Failed to cancel processing: ${error.message}`, 'danger');
    }
  }
  ...
  async validateLocation() {
    ...
    if (!location) {
      this.ui.setValidationState(locationInput, false, 'Please enter a location.');
      return;
    }
    ...
    try {
      const data = await this.api.validateLocation(location, locationType.value);
      ...
      if (data?.osm_id && data?.display_name) {
        ...
        if (resultDiv) {
          ...
          resultDiv.querySelector('.validation-message').textContent = `Found: ${data.display_name}`;
        }
        
        window.notificationManager.show(`Location validated: ${data.display_name}`, 'success');
      } else {
        this.ui.setValidationState(locationInput, false, 'Location not found. Please check your input.');
      }
    } catch (error) {
      console.error('Error validating location:', error);
      this.ui.setValidationState(locationInput, false, `Validation failed: ${error.message}`);
    } finally {
  ...
  async addCoverageArea() {
    if (!this.state.validatedLocation?.display_name) {
      window.notificationManager.show('Please validate a location first.', 'warning');
      return;
    }

    const addButton = this.ui.getElement('add-coverage-area');
    const modal = bootstrap.Modal.getInstance(this.ui.getElement('addAreaModal'));
    ...
    try {
      ...
      const exists = areas.areas.some(area => 
        area.location?.display_name === this.state.validatedLocation.display_name
      );

      if (exists) {
        window.notificationManager.show('This area is already being tracked.', 'warning');
        return;
      }
      ...
      if (taskData?.task_id) {
        ...
        window.notificationManager.show(`Processing completed for ${locationToAdd.display_name}`, 'success');
        await this.loadCoverageAreas();
      }
      ...
      window.notificationManager.show(`Failed to add coverage area: ${error.message}`, 'danger');
      await this.loadCoverageAreas();
    } finally {
  ...
  async displayCoverageDashboard(locationId) {
    ...
    try {
      ...
      const streetsData = await this.api.getStreets(locationId, { cache_bust: Date.now() });

      coverage.streets_geojson = streetsData;
      this.state.selectedLocation = coverage;

      locationName.textContent = coverage.location.display_name || 'Unnamed Area';
      ...
      window.notificationManager.show(`Error loading dashboard: ${error.message}`, 'danger');
    } finally {
      this.initTooltips();
    }
  }
  ...
  updateDashboardStats(coverage) {
    ...
    const stats = {
      totalLength: {
        value: this.ui.constructor.distanceInUserUnits(coverage.total_length || 0),
        label: 'Total Length'
      },
      drivenLength: {
        value: this.ui.constructor.distanceInUserUnits(coverage.driven_length || 0),
        label: 'Driven Length',
      ...
      coveredSegments: {
        value: this.calculateCoveredSegments(coverage.street_types || []).toLocaleString(),
        label: 'Driven Segments',
        className: 'text-success'
      },
      lastUpdated: {
        value: this.ui.constructor.formatRelativeTime(coverage.last_stats_update || coverage.last_updated),
        label: 'Last Updated',
  ...
  calculateCoveredSegments(streetTypes) {
    return streetTypes.reduce((sum, typeStats) => {
      const covered = parseInt(typeStats.covered || typeStats.covered_segments || 0, 10);
  ...
  updateStreetTypeCoverage(streetTypes) {
    ...
    if (!streetTypes.length) {
      container.innerHTML = this.ui.createAlert('secondary', 'No Data', 'No street type data available.');
      return;
    }

    const sortedTypes = streetTypes
      .sort((a, b) => parseFloat(b.total_length_m || 0) - parseFloat(a.total_length_m || 0))
      .slice(0, 6);

    const html = sortedTypes.map(type => {
      const coveragePct = parseFloat(type.coverage_percentage || 0).toFixed(1);
      const coveredDist = this.ui.constructor.distanceInUserUnits(type.covered_length_m || 0);
      const totalDist = this.ui.constructor.distanceInUserUnits(
        type.driveable_length_m !== undefined ? type.driveable_length_m : type.total_length_m || 0
      );
      
      const barColor = parseFloat(coveragePct) < 25 ? 'bg-danger' 
        : parseFloat(coveragePct) < 75 ? 'bg-warning' : 'bg-success';
  ...
  createStreetTypeChart(streetTypes) {
    ...
    if (!streetTypes.length) {
      chartContainer.innerHTML = this.ui.createAlert('secondary', 'No Data', 'No street type data for chart.');
      return;
    }
    ...
    new Chart(ctx, {
      type: 'bar',
      data: {
        labels: sortedTypes.map(t => this.ui.constructor.formatStreetType(t.type)),
        datasets: [{
          label: 'Covered (mi)',
          data: sortedTypes.map(t => (t.covered_length_m || 0) * 0.000621371),
  ...
  updateUndrivenStreetsList(geojson) {
    ...
    if (!undrivenData.length) {
      container.innerHTML = this.ui.createAlert('success', 'All Covered', 'Every street has at least one driven segment.');
      return;
    }
    ...
    // Build list HTML
    const html = undrivenData.map(item => {
  ...
  showStreetOnMap(streetName) {
    ...
    if (!matchingFeatures.length) {
      window.notificationManager.show(`No geometry found for '${streetName}'.`, 'warning');
      return;
    }
    ...
    if (!bounds.isEmpty()) {
      this.map.map.fitBounds(bounds, { padding: 40, maxZoom: 18, duration: 800 });
    }
  }
  ...
  updateFilterButtonStates() {
    const buttons = document.querySelectorAll('.map-controls button[data-filter]');
  ...
  clearDashboardUI() {
    this.ui.getElement('dashboard-location-name').textContent = 'Select a location';
    
    const statsContainer = document.querySelector('.dashboard-stats-card .stats-container');
    if (statsContainer) statsContainer.innerHTML = '';

    ['street-type-chart', 'street-type-coverage', 'undriven-streets-list'].forEach(id => {
  ...
  async handleMarkSegmentAction(action, segmentId) {
    const locationId = this.state.selectedLocation?._id || this.state.currentDashboardLocationId;
    if (!locationId || !segmentId) return;

    try {
      await this.api.markSegment(locationId, segmentId, action);
      window.notificationManager.show(`Segment marked as ${action}`, 'success', 2000);
      ...
      window.notificationManager.show(`Failed to mark segment: ${error.message}`, 'danger');
    }
  }
  ...
  toggleSegmentSelection(segmentId) {
    ...
    this.map.updateSelectionHighlight(Array.from(this.state.selectedSegmentIds));
  ...
  async handleBulkMarkSegments(action) {
    ...
    try {
      await Promise.allSettled(
        segmentIds.map(segId => this.api.markSegment(locationId, segId, action))
      );

      // Optimistic updates
      segmentIds.forEach(segId => this.updateSegmentInGeoJson(segId, action));

      window.notificationManager.show(`${segmentIds.length} segments marked as ${action}`, 'success');
      
      await this.refreshDashboardData(locationId);
      await this.loadCoverageAreas();
      this.clearSelection();
    } catch (error) {
      window.notificationManager.show(`Bulk operation failed: ${error.message}`, 'danger');
    }
  }
  ...
  async batchRefreshAll() {
    ...
    const progressModal = this.modal.createBatchProgressModal(areas.areas.length);
    progressModal.show();
  ...
  async exportAllCoverageData() {
    ...
    window.notificationManager.show('Coverage data exported successfully', 'success');
  }
    ...
    if (typeof html2canvas !== 'undefined') {
      ...
      html2canvas(mapContainer, {
        ...
        canvas.toBlob(blob => {
          ...
          const a = document.createElement('a');
          a.href = url;
          const locationName = this.state.selectedLocation?.location?.display_name || 'coverage_map';
      ...
      window.notificationManager.show('Export library not available.', 'warning');
    }
  }
  ...
  resetModalValidationState() {
    ...
    const drawingValidationResult = this.ui.getElement('drawing-validation-result');
  ...
  showInterruptedTaskNotification(progressData) {
    ...
    const notification = this.ui.createAlert('info', 'Interrupted Task Found', 
  ...
  async resumeInterruptedTask(savedData) {
    ...
    try {
      ...
      window.notificationManager.show(`Task for ${savedData.location.display_name} completed.`, 'success');
      await this.loadCoverageAreas();
      ...
      window.notificationManager.show(`Failed to resume task: ${error.message}`, 'danger');
      await this.loadCoverageAreas();
    }
  }
  ...
  async findMostEfficientStreets() {
    if (!this.state.selectedLocation?._id) {
      window.notificationManager.show('Please select a coverage area first.', 'warning');
      return;
    }
    ...
    try {
      ...
      window.notificationManager.show('Unable to determine current position.', 'warning');
      return;
    }
    ...
    try {
      const data = await this.api.getSuggestedStreets(this.state.selectedLocation._id, currentLat, currentLon);
      ...
      if (data.status === 'success' && data.suggested_clusters?.length > 0) {
        ...
        window.notificationManager.show(data.message || 'No efficient streets found.', 'info');
      }
    } catch (error) {
      console.error('Error finding efficient streets:', error);
      window.notificationManager.show(`Error finding efficient streets: ${error.message}`, 'danger');
    } finally {
  ...
  displayEfficientStreets(clusters) {
    ...
    window.notificationManager.show('Efficient streets displayed on map', 'info');
  }
  ...
  async validateCustomBoundary() {
    ...
    window.notificationManager.show('Custom boundary validation not implemented in streamlined version', 'info');
  }

  async addCustomCoverageArea() {
    // Simplified custom area addition
    window.notificationManager.show('Custom area addition not implemented in streamlined version', 'info');
  }

  async reprocessStreetsForArea(locationId) {
    // Simplified reprocessing
    window.notificationManager.show('Street reprocessing not implemented in streamlined version', 'info');
  }
}
