...
async function updateLocationAndOdometer() {
    ...
    try {
        ...
        if (data.latitude && data.longitude) {
            ...
            locationText.textContent = data.address || `${data.latitude.toFixed(6)}, ${data.longitude.toFixed(6)}`;
        } else {
...
function calculateTotalCost() {
    ...
    const pricePerGallon = parseFloat(document.getElementById('price-per-gallon').value) || 0;
...
function setupEventListeners() {
    ...
    document.getElementById('vehicle-select').addEventListener('change', async (e) => {
        selectedVehicle = vehicles.find(v => v.imei === e.target.value);
        await updateLocationAndOdometer();
        await loadRecentFillups();
        await loadStatistics();
    });
    ...
    // Fill-up time change
    document.getElementById('fillup-time').addEventListener('change', updateLocationAndOdometer);
    ...
    document.getElementById('price-per-gallon').addEventListener('input', calculateTotalCost);

    // Form submission
    document.getElementById('gas-fillup-form').addEventListener('submit', handleFormSubmit);
}
...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        const formData = {
            ...
            price_per_gallon: parseFloat(document.getElementById('price-per-gallon').value) || null,
...
async function loadRecentFillups() {
    ...
    try {
        ...
        if (fillups.length === 0) {
            fillupList.innerHTML = '<p class="text-center text-muted">No fill-ups recorded yet</p>';
            return;
        }
        ...
        fillupList.innerHTML = '<p class="text-center text-danger">Error loading fill-ups</p>';
    }
}
...
function createFillupItem(fillup) {
    ...
    const pricePerGallon = fillup.price_per_gallon ? `$${fillup.price_per_gallon.toFixed(2)}` : '--';
...
async function loadStatistics() {
    ...
    try {
        ...
        document.getElementById('total-spent').textContent = `$${(stats.total_cost || 0).toFixed(2)}`;
        document.getElementById('avg-mpg').textContent = stats.average_mpg ? stats.average_mpg.toFixed(1) : '--';
        document.getElementById('cost-per-mile').textContent = stats.cost_per_mile ? `$${stats.cost_per_mile.toFixed(3)}` : '$0.00';
