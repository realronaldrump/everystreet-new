...
class Sidebar {
	...
	initializeEventListeners() {
		...
		window.addEventListener('resize', this.debounce(
			this.handleResponsiveLayout.bind(this),
			250
		));
	...
	handleOutsideClick(e) {
		...
		if (isMobile && isOutsideClick && isSidebarActive && !this.elements.body.classList.contains('sidebar-collapsed')) {
			this.toggleSidebar();
		}
	}
	 ...
	 */
	loadSidebarState() {
		const isCollapsed = localStorage.getItem(this.config.storageKeys.sidebarState) === 'true';
		if (isCollapsed) {
			const {
				body,
				sidebar,
