...
- Trip filters: `start_date`, `end_date` (calendar date on `startTime`), `imei`,
  `status[]`, `include_invalid`
...
- GeoJSON exports include properties derived from current models and can include
  `geometry: null` if missing.
