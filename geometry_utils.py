...
def validate_coordinate(lon: float, lat: float) -> bool:
    ...
    """
    return MIN_LONGITUDE <= lon <= MAX_LONGITUDE and MIN_LATITUDE <= lat <= MAX_LATITUDE


def validate_coordinates_fast(
    coordinates: List[List[float]], sample_size: int = 10
...
def calculate_total_distance(
    coordinates: List[List[float]], unit: str = "miles", skip_invalid: bool = True
) -> float:
...
def create_linestring_geojson(
    coordinates: List[List[float]], properties: Optional[dict] = None
) -> dict:
...
def create_point_geojson(
    coordinate: List[float], properties: Optional[dict] = None
) -> dict:
