...
def _cleanup_old_metrics(metric_key: str, category: str):
    ...
    for collection_name in ["processing_times", "database_operations", "api_requests"]:
        if metric_key in _performance_data[collection_name]:
            ...
            _performance_data[collection_name][metric_key] = recent_metrics[
                -MAX_METRIC_HISTORY:
            ]
...
def get_optimization_report() -> Dict[str, Any]:
    ...
    slow_apis = [
        op for op, stats in api_perf.items() if stats.get("avg_duration_ms", 0) > 500
    ]
    if slow_apis:
    ...
    slow_db_ops = [
        op for op, stats in db_perf.items() if stats.get("avg_duration_ms", 0) > 200
    ]
    if slow_db_ops:
        report["recommendations"].append(
            {
...
def log_request_timing(
    endpoint: str, method: str, duration_ms: float, status_code: int
):
...
def get_real_time_metrics() -> Dict[str, Any]:
    ...
    return {
        ...
        "system_health": {
            ...
            "total_optimization_metrics": len(
                _performance_data["optimization_metrics"]
            ),
        },
    }
