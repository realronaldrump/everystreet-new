...
# --- WebSocket Live Trip Tracking Endpoints ---
...
async def get_active_trip_data():
    ...
    try:
        ...
        if live_trip and live_trip.get("gps"):
            ...
            return {
                "trip_id": live_trip.get("transactionId", "live-trip"),
        ...
        if gps_data.get("type") == "LineString":
            for i, point in enumerate(gps_data.get("coordinates", [])):
                ...
                if start_time and end_time and start_time < end_time:
                    ...
                    fraction = i / max(1, len(gps_data.get("coordinates", [])) - 1)
...
# --- Additional Endpoints to Fix Routing Issues ---
@app.post("/api/trips/fetch")
async def trips_fetch_redirect(request: Request):
    """Fetches only new trips since the most recent trip in the database"""
    try:
        # Find the most recent trip in the database across all collections
        recent_trips = []

        # Check all trip collections for most recent trip
        for collection in [
            trips_collection,
            matched_trips_collection,
            uploaded_trips_collection,
        ]:
            trip = await collection.find_one({}, sort=[("endTime", -1)])
            if trip and trip.get("endTime"):
                recent_trips.append(trip)

        # Determine start date from most recent trip or use fallback
        start_date = None
        if recent_trips:
            # Find the trip with the most recent endTime
            most_recent = max(recent_trips, key=lambda t: t.get("endTime"))
            start_date = most_recent["endTime"]
            # Ensure it's a datetime object with timezone
            if isinstance(start_date, str):
                start_date = dateutil_parser.isoparse(start_date)
            if start_date.tzinfo is None:
                start_date = start_date.replace(tzinfo=timezone.utc)

        # If no recent trips or start_date is somehow too old, use reasonable fallback
        if not start_date or start_date < datetime.now(timezone.utc) - timedelta(
            days=30
        ):
            # Default to 2 days ago if no trips found or date is too old
            start_date = datetime.now(timezone.utc) - timedelta(days=2)
            logger.info(
                f"No recent trips found or date too old. Using fallback date: {start_date.isoformat()}"
            )
        ...
        logger.info(
            f"Fetching trips from {start_date.isoformat()} to {end_date.isoformat()}"
        )

        # Fetch only new trips
        ...
        return {
            "status": "success",
            "message": f"Fetched trips since {start_date.isoformat()}",
            "trips_count": len(fetched_trips) if fetched_trips else 0,
        }
    except Exception as e:
