...
def calculate_street_coverage(boundary_geojson, streets_geojson, matched_trips):
    try:
        ...
        trips_utm = gpd.GeoDataFrame(
            geometry=[all_trips], crs=4326).to_crs(epsg=utm_epsg)
        trips_buffer = trips_utm.geometry.buffer(
            buffer_distance_meters).unary_union
        ...
        for chunk_start in range(0, len(streets_utm), CHUNK_SIZE):
            ...
            for idx, street in streets_chunk.iterrows():
                ...
                street_4326 = gpd.GeoSeries(
                    [street_geom], crs=utm_epsg).to_crs(4326)[0]
                features.append({
                    "type": "Feature",
