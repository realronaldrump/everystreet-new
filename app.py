    ...
    try:
        ...
        if event_type == "tripStart":
            ...
            if transaction_id in active_trips:
                active_trips[transaction_id]["data"].extend(
            ...
            # Update trip metrics
            if transaction_id in active_trips:
                active_trips[transaction_id]["metrics"] = webhook_data.get(
                    "metrics")
            emit_data = {
                "transactionId": transaction_id,
                "imei": webhook_data.get("imei"),
...
def store_trip_data(trip_data):
    ...
    try:
        ...
        start_time = datetime.fromisoformat(trip_data.get(
            "start", {}).get("timestamp")).replace(tzinfo=timezone.utc)
        end_time = datetime.fromisoformat(trip_data.get(
            "end", {}).get("timestamp")).replace(tzinfo=timezone.utc)
