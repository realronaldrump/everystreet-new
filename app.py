    ...
    try:
        ...
        for task_id in tasks_to_run:
            try:
                ...
                logger.error(
                    f"Error running task {task_id}: {e}", exc_info=True)
                results.append({
                    "task": task_id,
                    "status": "error",
                    "message": str(e)
...
async def get_street_coverage_cached(location_name: str):
    ...
    try:
        ...
        if not coverage_data:
            raise HTTPException(
                status_code=404, detail="No coverage data found for this location")
...
async def get_street_coverage(request: Request, background_tasks: BackgroundTasks):
    try:
        ...
        is_stale = False
        if cached_data:
            age = datetime.now(timezone.utc) - cached_data.get("last_updated",
        ...
        if not result:
            raise HTTPException(
                status_code=404, detail="Coverage computation failed")
...
async def ws_live_trip(websocket: WebSocket):
    ...
    try:
        ...
        logger.error(
            f"Error establishing WebSocket connection: {e}", exc_info=True)
        try:
