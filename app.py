...
async def fetch_and_store_trips_in_range(start_date, end_date, update_progress=False):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as sess:
            ...
            if not token:
                logger.error(
                    "No Bouncie token retrieved. Aborting trip fetch.")
                return
            ...
            for idx, imei in enumerate(AUTHORIZED_DEVICES, 1):
                ...
                logger.info(
                    f"Fetched {len(dev_trips)} trips for device {imei}")

                if update_progress:
                    progress_data["fetch_and_store_trips_in_range"]["progress"] = int(
            ...
            for index, trip in enumerate(all_trips):
                try:
                    ...
                    trip["startTime"] = parse_bouncie_timestamp(
                        trip.get("startTime"))
                    trip["endTime"] = parse_bouncie_timestamp(
                        trip.get("endTime"))

                    # Ensure timestamps are not None
                    if not trip["startTime"] or not trip["endTime"]:
                        logger.warning(
                            f"Skipping trip {trip['transactionId']} due to missing timestamps.")
                        error_count += 1
                        continue
                    ...
                    if store_trip(processed_trip):
                        processed_count += 1
                        logger.info(
                            f"Stored trip {trip['transactionId']} successfully.")

                    if update_progress:
                        progress_data["fetch_and_store_trips_in_range"]["progress"] = int(
                            50 + (index / total_trips) * 50)
        ...
        logger.error(
            f"Error in fetch_and_store_trips_in_range: {e}", exc_info=True)
        if update_progress:
            progress_data["fetch_and_store_trips_in_range"]["status"] = "failed"
            progress_data["fetch_and_store_trips_in_range"]["message"] = f"Error: {e}"
