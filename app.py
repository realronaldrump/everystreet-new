...
async def bouncie_webhook():
    ...
    try:
        ...
        if event_type == "tripStart":
            ...
            logger.debug(
                f"Webhook tripStart event received for transactionId: {txid}")
            ...
            logger.debug(
                f"Webhook tripData event received for transactionId: {txid}")
            ...
            try:
                ...
                if processed_trip:
                    is_valid, validation_message = validate_trip_data(
                        processed_trip)
                    if is_valid:
                        # Use existing store_trip function
                        stored = store_trip(processed_trip)
                        if stored:
                            realtime_data_collection.delete_many(
                                {"transactionId": txid})  # Cleanup after successful store
                            socketio.emit(
                                "trip_ended", {"transactionId": txid})
                            logger.info(
                                f"Trip {txid} finalized and stored from webhook data.")
                        else:
                            logger.error(
                                f"Failed to store trip {txid} from webhook data.")
                            return jsonify({"status": "error", "message": f"Failed to store trip {txid}."}), 500
                    else:
                        logger.error(
                            f"Invalid trip data assembled from webhook for {txid}: {validation_message}")
