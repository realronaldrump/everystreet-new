...
async def delete_matched_trip(trip_id: str):
    try:
        ...
        logger.error("Error deleting matched trip %s: %s",
...
async def process_and_store_trip(trip):
    try:
        ...
        logger.warning("Duplicate trip ID %s, skipping.",
                       trip["transactionId"])
    except Exception as e:
...
async def bulk_delete_uploaded_trips(request: Request):
    try:
        ...
        logger.error("Error in bulk_delete_uploaded_trips: %s",
                     e, exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_non_custom_places_visits():
    try:
        ...
        logger.error("Error fetching non-custom place visits: %s",
                     e, exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_task_details(task_id: str):
    ...
    try:
        ...
        logger.error("Error getting task details for %s: %s",
