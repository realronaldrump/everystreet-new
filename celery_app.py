...
logger.info(f"Configuring Celery with broker: {REDIS_URL.split('@')[-1] if '@' in REDIS_URL else REDIS_URL}")
...
app.conf.update(
    ...
    beat_schedule={
        "fetch_trips_hourly": {
            ...
            "schedule": timedelta(minutes=int(os.getenv("TRIP_FETCH_INTERVAL_MINUTES", "60"))),
