...
def process_webhook_event_task(self, data: Dict[str, Any]):
    ...
    try:
        if live_trips_coll_task is None or archived_live_trips_coll_task is None:
            logger.warning(
            ...
            live_collection = db_manager.get_collection("live_trips")
            archive_collection = db_manager.get_collection("archived_live_trips")
            initialize_live_tracking_db_for_task(live_collection, archive_collection)
            if live_trips_coll_task is None or archived_live_trips_coll_task is None:
                logger.critical(
        ...
        if (
            ...
            logger.error(
                "Cannot retry task %s as retry context is missing.", celery_task_id
            )
