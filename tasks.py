    ...
    try:
        ...
        for task_id_to_run in tasks_to_trigger:
            try:
                ...
                priority_name = TASK_METADATA[task_id_to_run]["priority"].name.lower()
                queue = (
                ...
                await asyncio.sleep(0.1)  # Small delay to allow tasks to be picked up
...
async def _send_manual_task(task_name: str, task_mapping: dict) -> Dict[str, Any]:
    ...
    try:
        ...
        queue = f"{priority_name}_priority" if priority_name != "unknown" else "default"
