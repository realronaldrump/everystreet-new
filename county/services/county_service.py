...
class CountyService:
    """County map and cache helpers."""

    @staticmethod
    async def get_county_topology(projection: str | None = None) -> dict[str, Any]:
        return await get_county_topology(projection)

    @staticmethod
    async def get_visited_counties() -> dict[str, Any]:
        return await get_visited_counties()

    @staticmethod
    async def recalculate_visited_counties(
        background_tasks: BackgroundTasks,
    ) -> dict[str, Any]:
        return await recalculate_visited_counties(background_tasks)
