...
@pytest.mark.asyncio
async def test_monitor_rotates_legacy_simulator_auth_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    update = AsyncMock(return_value=True)
    updated = {
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value={
                "redirect_uri": "https://www.everystreet.me/api/bouncie/callback",
                "webhook_key": bouncie_webhooks.LEGACY_SIMULATOR_WEBHOOK_KEY,
            },
        ),
    )
    monkeypatch.setattr(
            ...
            return_value=[
                {
                    ...
                    "name": bouncie_webhooks.LIVE_TRIP_WEBHOOK_NAME,
                    "url": "https://www.everystreet.me/bouncie-webhook",
                    "authKey": bouncie_webhooks.LEGACY_SIMULATOR_WEBHOOK_KEY,
                    "events": ["tripStart", "tripData", "tripMetrics", "tripEnd"],
                    "active": True,
                    "updatedAt": "2026-03-05T18:04:00Z",
                }
    ...
    monkeypatch.setattr(
        bouncie_webhooks.secrets,
        "token_urlsafe",
        lambda _size: "rotated-key",
    )
    ...
    assert result["status"] == "success"
    assert result["action"] == "updated"
    assert update_webhook.await_args.kwargs["auth_key"] == "rotated-key"
    assert update.await_args.args[0]["webhook_key"] == "rotated-key"
