...
async def serialize_live_trip(trip_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "startTime" in serialized:
        ...
        try:
            if isinstance(start_time_value, datetime):
                ...
                if "Z" in start_time_value:
                    ...
                    "+" not in start_time_value
                    and "-" not in start_time_value[10:]
                ):  # Basic check if tz offset exists
            ...
            if isinstance(start_time_obj, datetime):
                ...
                if start_time_obj.tzinfo is None:
                    start_time_obj = start_time_obj.replace(
                        tzinfo=timezone.utc
                    )
        ...
        logger.error(
            "startTime field missing during serialization for trip %s",
            serialized.get("transactionId", "N/A"),
        )
