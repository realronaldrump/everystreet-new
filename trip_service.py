        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if progress_tracker and progress_key:
                    ...
                    progress_tracker[progress_key]["message"] = f"Completed {func.__name__}"
                return result
            except Exception as e:
                if progress_tracker and progress_key:
                    progress_tracker[progress_key]["status"] = "failed"
                    progress_tracker[progress_key]["message"] = f"Failed {func.__name__}: {str(e)}"
                raise
        return wrapper
    return decorator
        ...
        for i, trip in enumerate(trips):
            ...
            try:
                ...
                if state == TripState.VALIDATED.value:
                    ...
                    result.geocoded += 1
                elif state == TripState.MAP_MATCHED.value or state == TripState.COMPLETED.value:
                ...
                result.errors.append({
                    "trip_id": trip.get("transactionId", "unknown"),
                    "error": str(e)
                })
                logger.error(
    ...
    async def refresh_geocoding(
        ...
        for trip_id in trip_ids:
            try:
                ...
                await self.process_single_trip(trip, options, trip.get("source", "unknown"))
                results["updated"] += 1
