...
class CoverageCalculator:
    ...
    def _calculate_optimal_batch_size(self) -> int:
        ...
        optimal_size = max(
            50, min(500, int(available_memory / (typical_trip_size * 2))))
        return optimal_size
    ...
    def initialize_projections(self):
        ...
        if projection_key not in CoverageCalculator._projection_cache:
            ...
            CoverageCalculator._projection_cache[projection_key] = (
                self.utm_proj, self.project_to_utm, self.project_to_wgs84)
        else:
            self.utm_proj, self.project_to_utm, self.project_to_wgs84 = CoverageCalculator._projection_cache[
                projection_key]
    ...
    async def build_spatial_index(self, streets: List[Dict[str, Any]]):
        ...
        if location_key not in CoverageCalculator._spatial_index_cache:
            ...
            CoverageCalculator._spatial_index_cache[location_key] = (
                self.streets_index, self.streets_lookup)
        else:
            self.streets_index, self.streets_lookup = CoverageCalculator._spatial_index_cache[
                location_key]
