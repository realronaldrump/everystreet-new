    ...
    async def build_spatial_index_and_stats(
        ...
        if boundary_geojson_data:
            try:
                ...
                ) and boundary_geojson_data.get("type") in ["Polygon", "MultiPolygon"]:
    ...
    try:
        ...
        result = await asyncio.wait_for(
            calculator.compute_coverage(
                run_incremental=False, boundary_geojson_data=boundary_data_for_calc
            ),
...
async def compute_incremental_coverage(
    ...
    try:
        ...
        result = await asyncio.wait_for(
            calculator.compute_coverage(
                run_incremental=True, boundary_geojson_data=boundary_data_for_calc
            ),
