...
async def process_gps_data_field(
    ...
    if isinstance(current_data, list):
        ...
        if len(unique_coords) == 1:
            ...
            logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): List input resulted in no valid unique points.")
        ...
        if geom_type == "Point":
            if isinstance(coordinates_raw, list) and len(coordinates_raw) == 2:
                try:
                    ...
                    if -180 <= lon_f <= 180 and -90 <= lat_f <= 90:
                        valid_coords_for_dict.append([lon_f, lat_f]) # For Point, this list will have 1 item
                    else:
                         logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): Point coordinates {coordinates_raw} out of WGS84 range.")
                except (ValueError, TypeError):
                    logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): Point coordinates {coordinates_raw} not valid numbers.")
            else:
                logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): Point 'coordinates' field malformed: {coordinates_raw}")

            if valid_coords_for_dict: # Should be exactly one pair
                 return {"type": "Point", "coordinates": valid_coords_for_dict[0]}, True, False
            else:
                 logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): Could not form valid Point from dict: {current_data}")
                 return None, True, False

        elif geom_type == "LineString":
            if isinstance(coordinates_raw, list) and len(coordinates_raw) >= 1: # Allow LineString with 1 point for now, will be converted
                seen_tuples = set()
                    ...
                    if isinstance(p_coord, list) and len(p_coord) == 2:
                        try:
                            ...
                            if -180 <= lon_f <= 180 and -90 <= lat_f <= 90:
                                ...
                                logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): LineString coord {p_coord} out of WGS84 range.")
                        except (ValueError, TypeError):
                            logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): LineString coord {p_coord} not valid numbers.")
                    else:
                        logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): Malformed point in LineString coordinates: {p_coord}")
            else:
                logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): LineString 'coordinates' field malformed or empty: {coordinates_raw}")

            if len(valid_coords_for_dict) == 1:
                return {"type": "Point", "coordinates": valid_coords_for_dict[0]}, True, False
            elif len(valid_coords_for_dict) >= 2:
                return {"type": "LineString", "coordinates": valid_coords_for_dict}, True, False
            else:
                logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): Could not form valid LineString/Point from dict: {current_data}")
                return None, True, False
            ...
            logger.warning(f"Doc {doc_id} ({field_name_logging_tag}): Dict is not Point/LineString (type: {geom_type}). Trying to process its 'coordinates' field if present.")
            if coordinates_raw and isinstance(coordinates_raw, list):
                ...
                return await process_gps_data_field(coordinates_raw, doc_id, f"{field_name_logging_tag}[coordinates_fallback]")
            logger.error(f"Doc {doc_id} ({field_name_logging_tag}): Dict is not Point/LineString and 'coordinates' field is not a processable list.")
            return None, True, True # Error, needs update (to unset)
...
async def migrate_collections_gps_data():
    ...
    CONFIGURATIONS = [
        {"collection_name": "trips", "field_name": "gps", "index_name": "gps_2dsphere_index"},
        {"collection_name": "matched_trips", "field_name": "matchedGps", "index_name": "matchedGps_2dsphere_index"}
    ]
    ...
    for config in CONFIGURATIONS:
        ...
        try:
            ...
            overall_summary.append({
                "collection": collection_name, "field": field_name, "status": "Failed to get collection", "error": str(e)
            })
            continue
        ...
        async for doc in cursor:
            ...
            processed_gps_data, needs_update, is_error_flag = await process_gps_data_field(
                gps_data_raw, doc_id, logging_tag
            )
            ...
            if needs_update:
                if processed_gps_data is not None:
                    ...
                    if isinstance(gps_data_raw, dict) and processed_gps_data == gps_data_raw and is_valid_geojson_object(processed_gps_data):
                    ...
                    documents_to_update_ops.append(
                        pymongo.UpdateOne({"_id": doc_id}, {"$set": {field_name: processed_gps_data}})
                    )
                    migrated_count += 1
            ...
            if len(documents_to_update_ops) >= batch_size:
                logger.info(f"Collection '{collection_name}': Writing batch of {len(documents_to_update_ops)} updates...")
                try:
                    await collection.bulk_write(documents_to_update_ops, ordered=False)
                except BulkWriteError as bwe:
                    logger.error(f"Bulk write error for '{collection_name}': {bwe.details}")
                    ...
                    logger.error(f"Error during bulk update for '{collection_name}': {e}")
                documents_to_update_ops = []
        
        # Write any remaining updates for the current collection
        if documents_to_update_ops:
            logger.info(f"Collection '{collection_name}': Writing final batch of {len(documents_to_update_ops)} updates...")
            try:
                await collection.bulk_write(documents_to_update_ops, ordered=False)
            except BulkWriteError as bwe:
                logger.error(f"Bulk write error for '{collection_name}' (final batch): {bwe.details}")
            except Exception as e:
                logger.error(f"Error during final bulk update for '{collection_name}': {e}")
        
        logger.info(f"--- Summary for Collection: '{collection_name}', Field: '{field_name}' ---")
        logger.info(f"Total documents inspected with field '{field_name}': {total_docs_inspected}")
        ...
        if migrated_count > 0 or (already_correct_count > 0 and total_docs_inspected > 0):
            logger.info(f"Attempting to create/ensure 2dsphere index '{index_name}' on '{collection_name}.{field_name}'...")
            try:
                ...
                current_config_summary["index_status"] = f"Successfully created/ensured '{index_name}'."
             ...
             logger.info(f"No documents with field '{field_name}' found in '{collection_name}'. Index creation skipped.")
...
if __name__ == "__main__":
    if not os.getenv("MONGO_URI"):
        ...
        if os.path.exists(".env"):
            logger.info("Found .env file, attempting to load MONGO_URI from it for local run.")
            load_dotenv(dotenv_path=".env", override=True) # Override to ensure it's picked up
            if not os.getenv("MONGO_URI"):
                 logger.error("MONGO_URI still not set after attempting to load .env. Exiting.")
                 exit(1)
            else:
